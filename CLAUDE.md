...
8. **Channel Layer** — Discord adapter (voice reverse RPC for gateway split), OpenAI-compatible API, admin GUI (admin UI: editable settings, model discovery, garden primer, memory type filters, session dates, full identity view, contacts/trust management, prompt soil)
