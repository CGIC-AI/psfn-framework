...
**Git tools have functional path allowlists.** Canonical allowlist: `['src/', 'docs/', 'psfn/']` (`src/security/policy-constants.ts:4`). `validatePath()` normalizes paths, blocks traversal (`..`), ensures path is inside repo root (`src/git/ops.ts:225-250`). Protected branch blocking enforced on `main`/`master` for write operations (line 137-139, 155, 169). Audit logging to JSONL (line 280-288). Shell command injection defense via `shellEscape()` with POSIX single-quote wrapping (line 276-278).
