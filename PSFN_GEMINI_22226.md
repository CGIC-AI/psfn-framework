# Substrate Audit Report: Purrsephone Substrate Framework (PSFN)
...
*   **Evidence**:
    ...
    *   **Filesystem**: `src/gateway/server.ts` uses `realpathSync` and prefix checking to prevent traversal. `src/git/ops.ts` uses an allowlist (`src/`, `docs/`, `purrsephone/`).
