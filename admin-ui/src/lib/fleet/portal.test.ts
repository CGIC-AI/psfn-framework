...
const CLUSTER_ICP = { state: 'inactive_singleton', activity: { status: 'not_applicable' } } as const;
const COMPANION_ICP = { state: 'not_applicable', reason: 'singleton_fleet', lifecycle: 'member' } as const;
...
describe('Garden fleet portal client', () => {
  ...
  it('queues the portal read behind a sibling session rotation', async () => {
    ...
    const portalRead = fetchFleetPortalProjection();
    await new Promise(resolve => setTimeout(resolve, 0));
    expect(requestedPaths).not.toContain('/v1/fleet/portal');
    ...
    await expect(portalRead).resolves.toMatchObject({ companions: [] });
