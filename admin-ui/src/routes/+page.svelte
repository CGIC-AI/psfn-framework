<script lang="ts">
  ...
  import type { AdminDashboardData, ThinkTraceView } from '$lib/types';
...
<div class="space-y-6">
  ...
  {:else if data}
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <!-- Memory by type -->
      <div class="card p-5">
        <h2 class="font-serif text-lg text-shadow-800 mb-3">Memory Breakdown</h2>
        <div class="space-y-2">
          {#each Object.entries(stats.memoryByType) as [type, count]}
            <div class="flex items-center gap-3">
              <span class="px-2 py-0.5 text-xs rounded border {memoryTypeColor(type)}">
                {type}
              </span>
              <div class="flex-1 h-2 bg-bark-200 rounded-full overflow-hidden">
                <div
      ...
      <!-- Quick nav -->
      <div class="card p-5">
        <h2 class="font-serif text-lg text-shadow-800 mb-3">Quick Navigation</h2>
    ...
    {#if stats.recentThinkTraces.length > 0}
      <div class="card p-5">
        <h2 class="font-serif text-lg text-shadow-800 mb-3">Recent Think Traces</h2>
