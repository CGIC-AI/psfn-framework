...
<div class="space-y-6">
  ...
  {:else if data}
    ...
    <section class="grid grid-cols-1 gap-6 xl:grid-cols-2">
      ...
      <div class="card-garden overflow-hidden"><div class="border-b border-bark-200 p-5"><h2 class="font-serif text-lg font-semibold text-shadow-900">Conversation cost breaker</h2><p class="text-sm text-shadow-600">Latest durable decision per conversation. Empty is expected when the breaker has no decisions.</p></div>{#if data.costs.length === 0}<p class="p-5 text-sm text-shadow-600">No cost-breaker decisions recorded.</p>{:else}<div class="divide-y divide-bark-200">{#each data.costs as cost (cost.conversationId)}<div class="p-4 text-sm"><div class="flex items-center justify-between gap-2"><span class={`rounded-full px-2 py-1 text-xs ${badge(costState(cost))}`}>{costState(cost).replaceAll('_', ' ')}</span><span class="text-xs text-shadow-500">{cost.reason.replaceAll('_', ' ')}</span></div><p class="mt-2 font-mono text-xs text-shadow-700">{cost.conversationId}</p><div class="mt-2 grid grid-cols-3 gap-2 text-xs text-shadow-600"><span>Actual {formatUsd(cost.actualCostUsd)}</span><span>Projected {formatUsd(cost.projectedTotalCostUsd)}</span><span>Hard {formatUsd(cost.hardLimitUsd)}</span></div></div>{/each}</div>{/if}</div>
    </section>
