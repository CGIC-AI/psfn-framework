...
<div class="space-y-6">
  ...
  {:else if data}
    ...
    <section class="grid grid-cols-1 gap-6 xl:grid-cols-2">
      ...
      <div class="card-garden overflow-hidden"><div class="border-b border-bark-200 p-5"><h2 class="font-serif text-lg font-semibold text-shadow-900">Conversation cost breaker</h2><p class="text-sm text-shadow-600">Latest durable decision per conversation. Empty is expected when the breaker has no decisions.</p></div>{#if !data.costProjection.available}<div class="border-l-4 border-l-gold-400 p-5"><p class="font-medium text-shadow-900">Cost projection unavailable</p><p class="mt-1 text-sm text-shadow-600">{data.costProjection.unavailableReason === 'relation_contract_unavailable' ? 'The optional fleet cost-decision relation is missing, malformed, or unreadable. The core autonomy control plane remains available.' : data.costProjection.unavailableReason === 'read_failed' ? 'The optional fleet cost-decision read failed. The core autonomy control plane remains available.' : 'The cost projection is unavailable because the ICP control plane is not provisioned.'}</p></div>{:else if data.costs.length === 0}<p class="p-5 text-sm text-shadow-600">No cost-breaker decisions recorded.</p>{:else}<div class="divide-y divide-bark-200">{#each data.costs as cost (cost.conversationId)}<div class="p-4 text-sm"><div class="flex items-center justify-between gap-2"><span class={`rounded-full px-2 py-1 text-xs ${badge(costState(cost))}`}>{costState(cost).replaceAll('_', ' ')}</span><span class="text-xs text-shadow-500">{cost.reason.replaceAll('_', ' ')}</span></div><p class="mt-2 font-mono text-xs text-shadow-700">{cost.conversationId}</p><div class="mt-2 grid grid-cols-3 gap-2 text-xs text-shadow-600"><span>Actual {formatUsd(cost.actualCostUsd)}</span><span>Projected {formatUsd(cost.projectedTotalCostUsd)}</span><span>Hard {formatUsd(cost.hardLimitUsd)}</span></div></div>{/each}</div>{/if}</div>
    </section>
