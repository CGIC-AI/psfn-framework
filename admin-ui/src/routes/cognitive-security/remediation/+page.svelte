<script lang="ts">
  // CogSec remediation console (htm9.11): relocated from the Session Recovery
  // page's former "CogSec" mode. Seals selected companion L0 rows, replaces
  ...
  let pendingApplyInput = $state<AdminCogSecRemediationInput | null>(null);
  ...
  async function loadRoutes(): Promise<void> {
    ...
    error = '';
    try {
      const [routeData, cogSecData] = await Promise.all([listSessionRoutes(), listCogSecEvents()]);
      routes = routeData.routes;
      channels = routeData.channels;
      cogSecEvents = cogSecData.events;
      if (!selectedSourceChannelId && routes.length > 0) {
        selectedSourceChannelId = routes[0].sourceChannelId;
      }
    } catch (e) {
  ...
  function requestApplyCogSec(): void {
    try {
      pendingApplyInput = buildCogSecInput();
...
<ConfirmationModal
  ...
  title="Apply CogSec remediation?"
  body={pendingApplyInput
    ? `Apply CogSec remediation to ${pendingApplyInput.sourceChannelId}? Selected companion L0 rows will be tombstoned, sealed evidence will be retained outside companion reach, and a fresh lane may be cut.`
    : ''}
  confirmLabel="Apply remediation"
  tone="danger"
  busy={cogSecSubmitting}
  onConfirm={() => void applyCogSec()}
  onCancel={() => { applyConfirmOpen = false; pendingApplyInput = null; }}
/>
