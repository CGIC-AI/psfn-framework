<script lang="ts">
  ...
  const TRUST_BADGE: Record<string, { class: string; label: string }> = {
    ...
    public:   { class: 'bg-shadow-100 text-shadow-600 border-shadow-300 dark:bg-shadow-800/30 dark:text-bark-400 dark:border-shadow-700', label: 'Public' },
  };
  ...
  const VERIFICATION_STATUS: Record<string, { class: string; label: string }> = {
    ...
    expired:  { class: 'bg-shadow-200 text-shadow-600 dark:bg-shadow-800/30 dark:text-bark-400', label: 'Expired' },
  };
      ...
      <div class="card-garden overflow-hidden">
        ...
        >
          <div>
            ...
            <p class="text-xs text-shadow-400 dark:text-bark-500 mt-0.5">Recent cross-channel identity verification challenges</p>
          </div>
            ...
            <table class="w-full text-sm">
              <thead>
                <tr class="bg-bark-50 dark:bg-shadow-800/50">
                  <th class="text-left px-4 py-2 text-xs font-medium text-shadow-500 dark:text-bark-400 uppercase tracking-wider">Status</th>
                  <th class="text-left px-4 py-2 text-xs font-medium text-shadow-500 dark:text-bark-400 uppercase tracking-wider">Source</th>
                  <th class="text-left px-4 py-2 text-xs font-medium text-shadow-500 dark:text-bark-400 uppercase tracking-wider">Target</th>
                  <th class="text-left px-4 py-2 text-xs font-medium text-shadow-500 dark:text-bark-400 uppercase tracking-wider">Contact</th>
                  <th class="text-left px-4 py-2 text-xs font-medium text-shadow-500 dark:text-bark-400 uppercase tracking-wider">Nonce</th>
                  <th class="text-left px-4 py-2 text-xs font-medium text-shadow-500 dark:text-bark-400 uppercase tracking-wider">Expiry</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-bark-100 dark:divide-shadow-800">
                {#each data.verifications.slice(0, 10) as v (v.id)}
                  ...
                  <tr class="hover:bg-bark-50/50 dark:hover:bg-shadow-800/30">
                    ...
                    <td class="px-4 py-2.5">
                      <span class="font-mono text-xs text-shadow-600 dark:text-bark-400">{v.sourceChannel}:{v.sourceUserId}</span>
                    </td>
                    <td class="px-4 py-2.5">
                      <span class="font-mono text-xs text-shadow-600 dark:text-bark-400">{v.targetChannel}:{v.targetUserId}</span>
                    </td>
                    <td class="px-4 py-2.5 text-xs text-shadow-600 dark:text-bark-400">{contactNameForId(v.contactId)}</td>
                    <td class="px-4 py-2.5">
                      <span class="font-mono text-[11px] text-shadow-400 dark:text-bark-500">{v.nonce ?? '-'}</span>
                    </td>
      ...
      <div class="card-garden overflow-hidden">
        ...
        >
          <div>
            ...
            <p class="text-xs text-shadow-400 dark:text-bark-500 mt-0.5">Persistent audit trail for trust level and contact note changes</p>
          </div>
          ...
          <div class="border-t border-bark-100 dark:border-shadow-800 overflow-x-auto">
            <table class="w-full text-sm">
              <thead>
                <tr class="bg-bark-50 dark:bg-shadow-800/50">
                  <th class="text-left px-4 py-2 text-xs font-medium text-shadow-500 dark:text-bark-400 uppercase tracking-wider">Contact</th>
                  <th class="text-left px-4 py-2 text-xs font-medium text-shadow-500 dark:text-bark-400 uppercase tracking-wider">Field</th>
                  <th class="text-left px-4 py-2 text-xs font-medium text-shadow-500 dark:text-bark-400 uppercase tracking-wider">Actor</th>
                  <th class="text-left px-4 py-2 text-xs font-medium text-shadow-500 dark:text-bark-400 uppercase tracking-wider">Old Value</th>
                  <th class="text-left px-4 py-2 text-xs font-medium text-shadow-500 dark:text-bark-400 uppercase tracking-wider">New Value</th>
                  <th class="text-left px-4 py-2 text-xs font-medium text-shadow-500 dark:text-bark-400 uppercase tracking-wider">Timestamp</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-bark-100 dark:divide-shadow-800">
                {#each data.mutationAudits as audit (audit.id)}
                  <tr class="hover:bg-bark-50/50 dark:hover:bg-shadow-800/30">
                    <td class="px-4 py-2.5">
                      <span class="font-mono text-xs text-shadow-600 dark:text-bark-400">{contactNameForId(audit.contactId)}</span>
                    </td>
                    <td class="px-4 py-2.5 text-xs text-shadow-600 dark:text-bark-400 capitalize">
                      {audit.field === 'trust_level' ? 'trust' : audit.field}
                    </td>
                    <td class="px-4 py-2.5">
                      <span class="font-mono text-xs text-shadow-500 dark:text-bark-400">{audit.actor}</span>
                    </td>
                    <td class="px-4 py-2.5">
                      {#if audit.oldValue}
                        <code class="text-xs bg-bark-100 dark:bg-shadow-800 px-1.5 py-0.5 rounded text-shadow-600 dark:text-bark-400">{audit.oldValue}</code>
                      {:else}
                        <span class="text-xs text-shadow-300 dark:text-bark-500 italic">empty</span>
                      {/if}
                    </td>
                    <td class="px-4 py-2.5">
                      {#if audit.newValue}
                        <code class="text-xs bg-bark-100 dark:bg-shadow-800 px-1.5 py-0.5 rounded text-shadow-600 dark:text-bark-400">{audit.newValue}</code>
                      {:else}
                        <span class="text-xs text-shadow-300 dark:text-bark-500 italic">empty</span>
                      {/if}
                    </td>
        ...
        {#if editingContact === contact.id}
          ...
          <div class="card-garden p-5 space-y-4 col-span-1 md:col-span-2 lg:col-span-3 filigree-border-strong ring-1 ring-gold-300">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label for="edit-display-name" class="block text-xs font-medium text-shadow-600 dark:text-bark-400 mb-1">Display Name</label>
                <input
              ...
              <div>
                <label for="edit-nickname" class="block text-xs font-medium text-shadow-600 dark:text-bark-400 mb-1">Nickname</label>
                <input
                  id="edit-nickname"
                  type="text"
              ...
              <div>
                <label for="edit-trust" class="block text-xs font-medium text-shadow-600 dark:text-bark-400 mb-1">Trust Level</label>
                <select
                  id="edit-trust"
              ...
              <div>
                <label for="edit-relationship" class="block text-xs font-medium text-shadow-600 dark:text-bark-400 mb-1">Relationship Type</label>
                <select
            ...
            <div>
              <label for="edit-notes" class="block text-xs font-medium text-shadow-600 dark:text-bark-400 mb-1">Notes</label>
              <textarea
                id="edit-notes"
            ...
            <div class="flex gap-2 pt-1">
              ...
              <button
                onclick={cancelEdit}
                class="px-4 py-2 rounded-lg text-shadow-500 dark:text-bark-400 text-sm hover:bg-bark-100 dark:hover:bg-shadow-800 transition-colors"
              >
                Cancel
              </button>
            </div>
          ...
          <div class="card-garden p-5 space-y-3">
            ...
            <div class="flex items-start justify-between">
              <div>
                ...
                {/if}
                <p class="text-[10px] font-mono text-shadow-300 dark:text-bark-500 mt-0.5">contact:{contact.id}</p>
              </div>
            ...
            {#if channels.length > 0}
              <div class="border-t border-bark-100 dark:border-shadow-800 pt-2">
                <p class="text-[11px] font-medium text-shadow-500 dark:text-bark-400 mb-1.5">Related Channels</p>
                <div class="flex flex-wrap gap-1.5">
                  {#each channels as ch}
                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] bg-bark-100 dark:bg-shadow-800 text-shadow-600 dark:text-bark-400 font-mono">
            ...
            {#if profile}
              <div class="border-t border-bark-100 dark:border-shadow-800 pt-2">
                <p class="text-[11px] font-medium text-shadow-500 dark:text-bark-400 mb-1">Profile</p>
                {#if profile.summary}
                  <p class="text-xs text-shadow-600 dark:text-bark-400 leading-relaxed">{profile.summary}</p>
                {/if}
                <div class="flex flex-wrap items-center gap-3 mt-1.5 text-[10px] text-shadow-400 dark:text-bark-500">
                ...
                {#if profile.sourceMemoryIds && profile.sourceMemoryIds.length > 0}
                  <details class="mt-1.5">
                    <summary class="text-[10px] text-shadow-400 dark:text-bark-500 cursor-pointer hover:text-gold-600 transition-colors">
                    ...
                    <div class="mt-1 flex flex-wrap gap-1">
                      {#each profile.sourceMemoryIds as memId}
                        <code class="text-[9px] bg-bark-100 dark:bg-shadow-800 px-1.5 py-0.5 rounded text-shadow-500 dark:text-bark-400 break-all">{memId}</code>
                      {/each}
                    </div>
                  </details>
                {/if}
            ...
            {:else}
              <div class="text-[11px] text-shadow-300 dark:text-bark-500 italic">No synthesized profile</div>
            {/if}
          </div>
        {/if}
      {:else}
