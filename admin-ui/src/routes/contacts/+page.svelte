<script lang="ts">
  ...
  const TRUST_BADGE: Record<string, { bg: string; text: string; border: string; label: string }> = {
    primary:  { bg: 'bg-gold-50', text: 'text-gold-700', border: 'border-gold-300', label: 'Primary' },
    trusted:  { bg: 'bg-moss-50', text: 'text-moss-700', border: 'border-moss-300', label: 'Trusted' },
    regular:  { bg: 'bg-bark-100', text: 'text-shadow-700', border: 'border-bark-300', label: 'Regular' },
    public:   { bg: 'bg-shadow-50', text: 'text-shadow-600', border: 'border-shadow-200', label: 'Public' },
  };

  const VERIFICATION_STATUS: Record<string, { cls: string; label: string }> = {
    pending:  { cls: 'bg-bark-200 text-shadow-700', label: 'Pending' },
    verified: { cls: 'bg-moss-100 text-moss-700', label: 'Verified' },
    failed:   { cls: 'bg-wilt-100 text-wilt-700', label: 'Failed' },
    expired:  { cls: 'bg-shadow-100 text-shadow-700', label: 'Expired' },
  };
  ...
  {#if saveMessage}
    <div class="px-4 py-2.5 rounded-lg text-sm font-medium
      {saveOk
        ...
        : 'bg-wilt-50 text-wilt-700 border border-wilt-200'}">
      {saveMessage}
    </div>
  {/if}
    ...
    {#if data.verifications.length > 0}
      <div class="card-garden overflow-hidden">
        ...
        >
          ...
          <span class="text-shadow-700 text-sm transition-transform {showVerifications ? 'rotate-180' : ''}">
            &#9660;
          </span>
        </button>
        {#if showVerifications}
          <div class="border-t border-bark-100 overflow-x-auto">
            <table class="w-full text-sm">
              <thead>
                <tr class="bg-bark-50">
                  <th class="text-left px-4 py-2.5 text-sm font-semibold text-shadow-700 uppercase tracking-wider">Status</th>
                  <th class="text-left px-4 py-2.5 text-sm font-semibold text-shadow-700 uppercase tracking-wider">Source</th>
                  <th class="text-left px-4 py-2.5 text-sm font-semibold text-shadow-700 uppercase tracking-wider">Target</th>
                  <th class="text-left px-4 py-2.5 text-sm font-semibold text-shadow-700 uppercase tracking-wider">Contact</th>
                  <th class="text-left px-4 py-2.5 text-sm font-semibold text-shadow-700 uppercase tracking-wider">Nonce</th>
                  <th class="text-left px-4 py-2.5 text-sm font-semibold text-shadow-700 uppercase tracking-wider">Expiry</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-bark-100">
                {#each data.verifications.slice(0, 10) as v (v.id)}
                  {@const vs = VERIFICATION_STATUS[v.status] ?? VERIFICATION_STATUS.expired}
                  <tr class="hover:bg-bark-50/50">
                    <td class="px-4 py-2.5">
                      <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {vs.cls}">
                        {vs.label}
                      </span>
                    </td>
    ...
    {#if data.mutationAudits.length > 0}
      <div class="card-garden overflow-hidden">
        ...
        >
          ...
          <span class="text-shadow-700 text-sm transition-transform {showAuditTrail ? 'rotate-180' : ''}">
            &#9660;
          </span>
        </button>
        {#if showAuditTrail}
          <div class="border-t border-bark-100 overflow-x-auto">
            <table class="w-full text-sm">
              <thead>
                <tr class="bg-bark-50">
                  <th class="text-left px-4 py-2.5 text-sm font-semibold text-shadow-700 uppercase tracking-wider">Contact</th>
                  <th class="text-left px-4 py-2.5 text-sm font-semibold text-shadow-700 uppercase tracking-wider">Field</th>
                  <th class="text-left px-4 py-2.5 text-sm font-semibold text-shadow-700 uppercase tracking-wider">Actor</th>
                  <th class="text-left px-4 py-2.5 text-sm font-semibold text-shadow-700 uppercase tracking-wider">Old Value</th>
                  <th class="text-left px-4 py-2.5 text-sm font-semibold text-shadow-700 uppercase tracking-wider">New Value</th>
                  <th class="text-left px-4 py-2.5 text-sm font-semibold text-shadow-700 uppercase tracking-wider">Timestamp</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-bark-100">
                {#each data.mutationAudits as audit (audit.id)}
                  <tr class="hover:bg-bark-50/50">
                    ...
                    <td class="px-4 py-2.5">
                      {#if audit.oldValue}
                        <code class="text-sm bg-bark-100 px-1.5 py-0.5 rounded text-shadow-800">{audit.oldValue}</code>
                      {:else}
                    ...
                    <td class="px-4 py-2.5">
                      {#if audit.newValue}
                        <code class="text-sm bg-bark-100 px-1.5 py-0.5 rounded text-shadow-800">{audit.newValue}</code>
                      {:else}
                        <span class="text-sm text-shadow-600 italic">empty</span>
                      {/if}
                    </td>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div class="card-garden p-5 flex flex-col gap-3">
              ...
              <div class="flex items-center gap-1.5 shrink-0">
                ...
                <button onclick={cancelTrustEdit}
                  class="px-2 py-1 text-sm font-medium rounded text-shadow-700
                         hover:bg-bark-100 transition-colors">
                  Cancel
                </button>
              </div>
            {:else}
          ...
          <div class="text-sm space-y-1">
            <p class="text-shadow-700">
              <span class="capitalize text-shadow-800">{contact.relationshipType.replace('_', ' ')}</span>
            </p>
          ...
          <div class="border-t border-bark-100 pt-2">
            {#if editingNotesId === contact.id}
              <div class="space-y-2">
                <textarea bind:value={editNotesValue} rows={3}
                  class="w-full px-3 py-2 rounded-lg border border-gold-300
                         bg-white text-shadow-800 text-sm resize-y
                         focus:outline-none focus:ring-2 focus:ring-gold-300"
                  placeholder="Notes about this contact..."
                ></textarea>
                <div class="flex gap-1.5">
                  <button onclick={() => saveNotes(contact.id)} disabled={saving}
                    class="px-2.5 py-1 text-sm font-medium rounded bg-gold-600 text-white
                           hover:bg-gold-700 disabled:opacity-50 transition-colors">
                    Save
                  </button>
                  <button onclick={cancelNotesEdit}
                    class="px-2.5 py-1 text-sm font-medium rounded text-shadow-700
                           hover:bg-bark-100 transition-colors">
                    Cancel
                  </button>
                </div>
            ...
            {:else}
              <button onclick={() => startNotesEdit(contact)}
                ...
                {#if contact.notes}
                  <p class="text-sm text-shadow-700 italic leading-relaxed
                            group-hover:text-shadow-900 transition-colors">
                    {contact.notes}
                  </p>
                {:else}
                ...
                <details class="mt-1.5">
                  ...
                  <div class="mt-1 flex flex-wrap gap-1">
                    {#each profile.sourceMemoryIds as memId}
                      <code class="text-sm bg-bark-100 px-1.5 py-0.5 rounded
                                   text-shadow-700 break-all">{memId}</code>
                    {/each}
                  </div>
                </details>
              {/if}
          ...
          {#if channels.length > 0}
            <div class="border-t border-bark-100 pt-2">
              <p class="text-sm font-semibold text-shadow-700 mb-1.5 uppercase tracking-wider">Channels</p>
              <div class="flex flex-wrap gap-1.5">
                {#each channels as ch}
                  <span class="inline-flex items-center px-2 py-0.5 rounded text-sm font-mono
                               bg-bark-100 text-shadow-700">
      ...
      {:else}
        <div class="col-span-full card-garden p-10 text-center">
          <p class="text-shadow-700 italic text-sm">No visitors have been seen in the garden yet</p>
        </div>
      {/each}
    </div>
  {/if}
