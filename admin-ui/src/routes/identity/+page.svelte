    ...
    {:else}
      <div class="space-y-4">
        ...
        <div class="card-garden p-5">
          <h3 class="text-sm font-serif font-semibold text-shadow-600 dark:text-shadow-400 mb-3">Import Character Card</h3>
          ...
          {#if importMessage}
            <p class="mt-2 text-xs text-shadow-500 dark:text-shadow-400">{importMessage}</p>
          {/if}
        </div>
        ...
        {#if data.history && data.history.length > 0}
          <div class="card-garden p-5">
            <h3 class="text-sm font-serif font-semibold text-shadow-600 dark:text-shadow-400 mb-3">Version History</h3>
            {#if rollbackMessage}
              <p class="mb-3 text-xs text-shadow-500 dark:text-shadow-400">{rollbackMessage}</p>
            {/if}
            <div class="space-y-2">
              {#each data.history as entry (entry.version)}
                ...
                <div class="rounded-lg border border-bark-200 dark:border-shadow-700 overflow-hidden">
                  <div class="flex items-center gap-3 px-4 py-2.5 bg-bark-50 dark:bg-shadow-800/50">
                    <span class="font-mono text-xs text-shadow-600 dark:text-shadow-400">v{entry.version}</span>
                    <span class="text-xs text-shadow-500 dark:text-shadow-400">{new Date(entry.timestamp).toLocaleString()}</span>
                    <span class="text-xs text-shadow-300 dark:text-shadow-600">{entry.changedBy}</span>
                    {#if isCurrent}
                    ...
                    {:else}
                      <div class="ml-auto flex gap-1.5">
                        <button
                          ...
                          class="px-2.5 py-1 text-[11px] font-medium rounded border border-bark-300 dark:border-shadow-600 text-shadow-500 dark:text-shadow-400 hover:bg-bark-100 dark:hover:bg-shadow-800 transition-colors"
                        >
                  ...
                  {#if diffVersion === entry.version}
                    <div class="border-t border-bark-100 dark:border-shadow-800 p-4">
                      ...
                      {:else if diffCurrent && diffTarget}
                        <p class="text-xs text-shadow-400 dark:text-shadow-500 mb-3">
                        ...
                        <div class="space-y-3">
                          {#each DIFF_FIELDS as df (df.key)}
                            ...
                            {#if differs}
                              <div>
                                <p class="text-xs font-medium text-shadow-500 dark:text-shadow-400 mb-1.5">{df.label}</p>
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                                  <div class="rounded-lg border border-bark-200 dark:border-shadow-700 p-3 bg-bark-50 dark:bg-shadow-900">
                                    <p class="text-[10px] font-medium text-shadow-400 dark:text-shadow-500 uppercase tracking-wider mb-1">Current (v{data.version})</p>
                          ...
                          {#if !DIFF_FIELDS.some(df => fieldsDiffer(diffCurrent!, diffTarget!, df.key))}
                            <p class="text-xs text-shadow-400 dark:text-shadow-500 italic">No field differences detected.</p>
                          {/if}
                        </div>
                      {:else}
                        <p class="text-xs text-shadow-400 dark:text-shadow-500 italic">Unable to load diff.</p>
                      {/if}
                    </div>
                  {/if}
                </div>
