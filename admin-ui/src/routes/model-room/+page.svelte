<script lang="ts">
  ...
  async function runRoomRound() {
    ...
    if (!includePSFN && activeParticipants.length === 0) {
      error = 'Enable PSFN or at least one model participant.';
      return;
    }
    ...
    try {
      if (includePSFN) {
        const purrReply = await requestTurn({
          speakerId: bootstrap.psfn.id,
          speakerName: bootstrap.psfn.displayName,
          input: baton,
          model: 'psfn',
          previousSpeakerName,
        });
        ...
        await scrollToBottom();
        baton = purrReply;
        previousSpeakerName = bootstrap.psfn.displayName;
      }
...
<div class="flex flex-col" style="height: calc(100vh - 6rem);">
  <div class="flex items-center justify-between mb-3 shrink-0">
    <div>
      ...
      <p class="text-shadow-600 text-sm mt-0.5">Multi-model room chat for PSFN + direct-provider personas</p>
    </div>
    <button
      ...
      <div class="flex flex-wrap gap-4 items-end">
        ...
        <label class="flex items-center gap-2 text-sm text-shadow-800 font-medium">
          <input
            type="checkbox"
            checked={includePSFN}
            onchange={() => includePSFN = !includePSFN}
          />
          Include {bootstrap.psfn.displayName}
        </label>
    ...
    >
      {#if messages.length === 0}
        <div class="flex items-center justify-center h-full">
          <p class="text-shadow-600 text-sm">Start a round to let PSFN and your selected models chat.</p>
        </div>
      {:else}
        <div class="space-y-3">
