    ...
    <div class="space-y-4 min-w-0">
      ...
      {:else}
        <div class="grid grid-cols-1 2xl:grid-cols-[minmax(0,1.1fr),minmax(0,0.9fr)] gap-4">
          ...
          <section class="card-garden overflow-hidden">
            {#if selectedTurn}
              {@const metrics = selectedTurnMetrics}
              <div class="border-b border-bark-300 bg-bark-100 px-5 py-4">
              ...
              <div class="space-y-5 p-5">
                <div class="grid grid-cols-2 gap-3 xl:grid-cols-4">
                  <div class="rounded-xl border border-bark-200 bg-white p-3">
                    <p class="text-sm text-shadow-600">Prompt Stage</p>
                  ...
                  <div class="rounded-xl border border-bark-200 bg-white p-3">
                    <p class="text-sm text-shadow-600">TTFT</p>
                  ...
                  <div class="rounded-xl border border-bark-200 bg-white p-3">
                    ...
                    <p class="mt-1 font-mono text-sm text-shadow-900">
                      {truncateValue(metrics?.promptVersionPointer, 16)}
                    </p>
                  </div>
                  <div class="rounded-xl border border-bark-200 bg-white p-3">
                    <p class="text-sm text-shadow-600">Static Hash</p>
                    <p class="mt-1 font-mono text-sm text-shadow-900">
                      {truncateValue(metrics?.staticHash, 16)}
                    </p>
                  </div>
                </div>
                ...
                <div class="rounded-xl border border-bark-200 bg-white p-4">
                  ...
                  <div class="mt-3 space-y-2">
                    {#each PROMPT_MONITOR_STAGE_ORDER as stageName}
                      {@const stage = selectedTurn.stages.find(candidate => candidate.stage === stageName)}
                      <div class="flex items-start justify-between gap-3 rounded-lg border px-3 py-2
                        ...
                        <div class="text-right">
                          ...
                          {#if stage}
                            <p class="mt-0.5 text-sm text-shadow-600">
                              {Object.entries(stage.data).length} field{Object.entries(stage.data).length === 1 ? '' : 's'}
                            </p>
                          {/if}
                        </div>
                      </div>
