<script lang="ts">
  ...
  const LAYER_BADGE: Record<string, { bg: string; text: string; label: string }> = {
    base:     { bg: 'bg-gold-100', text: 'text-gold-600', label: 'base' },
    operator: { bg: 'bg-moss-100', text: 'text-moss-600', label: 'operator' },
    runtime:  { bg: 'bg-shadow-100', text: 'text-shadow-700', label: 'runtime' },
    channel:  { bg: 'bg-petal-100', text: 'text-petal-600', label: 'channel' },
    task:     { bg: 'bg-bark-200', text: 'text-shadow-800', label: 'task' },
  };
...
<div class="space-y-6">
  ...
  <div>
    ...
    <p class="text-sm text-shadow-700 mt-1">Layered prompt stack -- {layers.length} layer{layers.length === 1 ? '' : 's'}</p>
  </div>
      ...
      {#each Array(5) as _}
        <div class="card-garden p-4 animate-pulse">
          <div class="flex items-center gap-3">
            <div class="h-5 w-14 bg-bark-200 rounded"></div>
            <div class="h-4 bg-bark-200 rounded flex-1"></div>
            <div class="h-4 w-8 bg-bark-200 rounded"></div>
          </div>
        </div>
      {/each}
    </div>
  {:else}
      ...
      <div class="space-y-2">
        {#each sortedLayers as layer (layer.id)}
          ...
          <div class="card-garden overflow-hidden transition-all {!layer.enabled ? 'opacity-60' : ''} {isSelected ? 'filigree-border-strong ring-1 ring-gold-300' : ''}">
            ...
            <div
              class="w-full text-left px-4 py-3 flex items-center gap-3 hover:bg-bark-50 transition-colors cursor-pointer"
            ...
            >
              ...
              {#if !locked}
                <div class="flex items-center gap-0.5 shrink-0" onclick={(e) => e.stopPropagation()}>
                  <button
                    ...
                    class="w-5 h-5 flex items-center justify-center rounded text-shadow-600 hover:text-gold-600 hover:bg-bark-100 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
              ...
              {#if editingPriorityId === layer.id}
                <div class="flex items-center gap-1 shrink-0" onclick={(e) => e.stopPropagation()}>
                  <input
                    ...
                    class="w-14 px-1.5 py-0.5 text-sm font-mono rounded border border-gold-300 bg-white text-shadow-900 text-center focus:outline-none focus:ring-1 focus:ring-gold-400"
                  />
              ...
              <!-- Content preview -->
              <span class="text-sm text-shadow-600 truncate max-w-32 hidden md:inline">{truncate(layer.content, 60)}</span>
              ...
              {:else}
                <button
                  ...
                  class="relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-gold-300 focus:ring-offset-2 {layer.enabled ? 'bg-moss-500' : 'bg-bark-300'}"
                  role="switch"
                    ...
                    <div>
                      ...
                      <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mt-2 text-xs">
                        ...
                        <div>
                          ...
                          <p class="mt-0.5 {dl.enabled ? 'text-moss-600' : 'text-wilt-500'}">{dl.enabled ? 'Yes' : 'No'}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      {#if showDiff}
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-2 mb-3">
                          <div class="rounded-lg border border-bark-200 p-3 bg-bark-50">
                            <p class="text-sm font-medium text-shadow-600 uppercase tracking-wider mb-1.5">Previous</p>
                            <pre class="text-xs font-mono text-shadow-800 whitespace-pre-wrap max-h-48 overflow-y-auto">{diffOld || '(empty)'}</pre>
                          </div>
                          <div class="rounded-lg border border-gold-200 p-3 bg-gold-50">
                            <p class="text-sm font-medium text-gold-600 uppercase tracking-wider mb-1.5">Current</p>
                            <pre class="text-xs font-mono text-shadow-800 whitespace-pre-wrap max-h-48 overflow-y-auto">{diffNew || '(empty)'}</pre>
                          </div>
                        </div>
                      {/if}
                      ...
                      {#if editingContent}
                        <div class="relative">
                          <textarea
                            ...
                            rows={14}
                            class="w-full px-3 py-2 rounded-lg border border-bark-300 bg-bark-50 text-shadow-900 text-xs font-mono resize-vertical leading-relaxed
                          ...
                          <div class="flex items-center justify-between mt-2">
                            ...
                            <div class="flex gap-2">
                              <button
                                ...
                                class="px-3 py-1.5 rounded-lg text-shadow-700 text-xs hover:bg-bark-100 transition-colors"
                              >
                                Cancel
                              </button>
                              <button
                      ...
                      {:else}
                        ...
                        <div class="relative">
                          {#if locked}
                            <div class="absolute top-2 right-2 flex items-center gap-1 px-2 py-0.5 rounded text-sm bg-bark-100 text-shadow-600">
                          ...
                          {/if}
                          <pre class="text-xs font-mono text-shadow-800 whitespace-pre-wrap bg-bark-50 p-3 rounded-lg max-h-72 overflow-y-auto leading-relaxed {locked ? 'border border-bark-200' : ''}">{dl.content}</pre>
                          <span class="block text-sm text-shadow-600 mt-1 text-right">{dl.content.length} characters</span>
                        </div>
                      {/if}

                      {#if saveMessage}
                        <p class="mt-2 text-xs text-moss-600">{saveMessage}</p>
                      {/if}
                    </div>
                      ...
                      <div>
                        ...
                        <div class="space-y-1.5">
                          {#each detailData.layerHistory as entry (entry.version)}
                            ...
                            <div class="flex items-center gap-3 px-3 py-2 rounded-lg text-xs {isCurrent ? 'bg-gold-50/50 border border-gold-200' : 'bg-bark-50'}">
                              <span class="font-mono font-medium {isCurrent ? 'text-gold-600' : 'text-shadow-800'}">v{entry.version}</span>
                              ...
                              {:else}
                                <button
                                  ...
                                  class="ml-auto px-2.5 py-0.5 text-sm font-medium rounded border border-wilt-300 text-wilt-600 hover:bg-wilt-50 transition-colors disabled:opacity-50"
                                >
    ...
    {#if staticPrompts.length > 0}
      ...
      <div>
        ...
        <p class="text-xs text-shadow-600 mb-3">{staticPrompts.length} registered prompt{staticPrompts.length === 1 ? '' : 's'} -- always active</p>
        ...
        <div class="space-y-2">
          {#each staticPrompts as sp (sp.key)}
            ...
            <div class="card-garden overflow-hidden {!sp.enabled ? 'opacity-60' : ''}">
              <!-- svelte-ignore a11y_no_static_element_interactions -->
              <div
                class="px-4 py-3 flex items-center gap-3 hover:bg-bark-50 transition-colors cursor-pointer"
                ...
                tabindex="0"
              >
                <span class="px-2 py-0.5 rounded text-sm font-bold uppercase tracking-wide bg-bark-100 text-shadow-700">
                  static
                </span>
                ...
                <span class="flex-1"></span>
                {#if sp.category}
                  <span class="text-sm px-1.5 py-0.5 rounded-full bg-bark-100 text-shadow-700">{sp.category}</span>
                {/if}
              ...
              {#if isExpanded}
                <div class="border-t border-bark-100 p-4 space-y-3">
                  <div class="flex items-center gap-4 text-xs text-shadow-700">
                    <span>Updated: {new Date(sp.updatedAt).toLocaleString()}</span>
                    {#if sp.enabled}
                      <span class="text-moss-600">Enabled</span>
                    {:else}
                      <span class="text-wilt-500">Disabled</span>
                    {/if}
                  </div>
                  <pre class="text-xs font-mono text-shadow-800 whitespace-pre-wrap bg-bark-50 p-3 rounded-lg max-h-64 overflow-y-auto leading-relaxed">{sp.content}</pre>
