...
<div class="space-y-4">
  ...
  {#if error}
    <div class="card-garden p-4 text-wilt-600 dark:text-wilt-400 text-sm">
      {error}
      <button onclick={() => error = ''} class="ml-2 text-shadow-400 hover:text-shadow-600 dark:text-bark-500 dark:hover:text-shadow-300">&times;</button>
    </div>
  {/if}
                ...
                <div class="p-4 space-y-4">
                  ...
                  <div>
                    <div class="flex items-center justify-between mb-2">
                      <span class="text-[11px] font-medium text-shadow-400 dark:text-bark-500 uppercase tracking-wider">Metadata</span>
                    ...
                    {#if editingMeta}
                      <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                        <div>
                          <label for="edit-layer-name" class="block text-xs text-shadow-500 dark:text-bark-400 mb-1">Name</label>
                          <input
                            id="edit-layer-name"
                            type="text"
                            bind:value={editName}
                        ...
                        <div>
                          <label for="edit-layer-priority" class="block text-xs text-shadow-500 dark:text-bark-400 mb-1">Priority</label>
                          <input
                        ...
                        <div>
                          <label for="edit-layer-role" class="block text-xs text-shadow-500 dark:text-bark-400 mb-1">Role</label>
                          <select
                            id="edit-layer-role"
                            bind:value={editRole}
                            class="w-full px-3 py-2 rounded-lg border border-bark-300 dark:border-shadow-600 bg-bark-50 dark:bg-shadow-800 text-shadow-900 dark:text-bark-200 text-sm
                  ...
                  <div>
                    <div class="flex items-center justify-between mb-2">
                      <span class="text-[11px] font-medium text-shadow-400 dark:text-bark-500 uppercase tracking-wider">Content</span>
                    ...
                    {#if showDiff}
                      <div class="grid grid-cols-1 md:grid-cols-2 gap-2 mb-3">
                        <div class="rounded-lg border border-bark-200 dark:border-shadow-700 p-3 bg-bark-50 dark:bg-shadow-900">
                          <p class="text-[10px] font-medium text-shadow-400 dark:text-bark-500 uppercase tracking-wider mb-1">Previous</p>
                          <pre class="text-xs font-mono text-shadow-600 dark:text-bark-400 whitespace-pre-wrap max-h-48 overflow-y-auto">{diffOld || '(empty)'}</pre>
                        </div>
                        <div class="rounded-lg border border-gold-200 dark:border-gold-800 p-3 bg-gold-50/50 dark:bg-gold-900/10">
                          ...
                          <pre class="text-xs font-mono text-shadow-600 dark:text-bark-400 whitespace-pre-wrap max-h-48 overflow-y-auto">{diffNew || '(empty)'}</pre>
                        </div>
                      </div>
                    {/if}
                    ...
                    {:else}
                      <pre class="text-xs font-mono text-shadow-600 dark:text-bark-400 whitespace-pre-wrap bg-bark-50 dark:bg-shadow-900 p-3 rounded-lg max-h-64 overflow-y-auto">{dl.content}</pre>
                    {/if}
                  </div>
                  ...
                  {#if detailData.layerHistory && detailData.layerHistory.length > 0}
                    <div>
                      <span class="text-[11px] font-medium text-shadow-400 dark:text-bark-500 uppercase tracking-wider">Version History</span>
                      <div class="mt-2 space-y-1.5">
                        {#each detailData.layerHistory as entry (entry.version)}
                          <div class="flex items-center gap-3 px-3 py-2 rounded-lg bg-bark-50 dark:bg-shadow-800/50 text-xs">
                            <span class="font-mono text-shadow-600 dark:text-bark-400">v{entry.version}</span>
                            <span class="text-shadow-500 dark:text-bark-400">{new Date(entry.timestamp).toLocaleString()}</span>
                            <span class="text-shadow-300 dark:text-bark-500">{entry.updatedBy}</span>
                            {#if entry.reason}
                              <span class="text-shadow-400 dark:text-bark-500 italic truncate">{entry.reason}</span>
                            {/if}
            ...
            <details class="card-garden overflow-hidden {!sp.enabled ? 'opacity-50' : ''}">
              <summary class="px-4 py-3 cursor-pointer hover:bg-bark-50 dark:hover:bg-shadow-800/50 transition-colors">
                <div class="inline-flex items-center gap-3">
                  ...
                  <span class="text-[11px] text-shadow-300 dark:text-bark-500">{new Date(sp.updatedAt).toLocaleString()}</span>
                </div>
              </summary>
              <div class="px-4 pb-4 border-t border-bark-100 dark:border-shadow-800">
                <pre class="mt-3 text-xs font-mono text-shadow-600 dark:text-bark-400 whitespace-pre-wrap bg-bark-50 dark:bg-shadow-900 p-3 rounded-lg max-h-48 overflow-y-auto">{sp.content}</pre>
              </div>
            </details>
          {/each}
        </div>
