<script lang="ts">
  ...
  function verificationBadgeClass(verification: string): string {
    switch (verification) {
      ...
      case 'missing_hash':
      default:
        return 'bg-shadow-100 text-shadow-500 dark:bg-shadow-800 dark:text-shadow-400 border-shadow-300 dark:border-shadow-600';
    }
  }
...
<div class="space-y-4">
  <div>
    ...
    <p class="text-sm text-shadow-400 dark:text-shadow-500 mt-1">Conversation sessions across all channels</p>
  </div>
  ...
  <div class="flex gap-4 h-[calc(100vh-12rem)]">
    ...
    <div class="w-80 shrink-0 card-garden overflow-y-auto">
      ...
      {:else}
        <div class="divide-y divide-bark-100 dark:divide-shadow-800">
            ...
            >
              <div class="flex items-center gap-2">
                ...
                {#if channelTypeTag(ch.channelId)}
                  <span class="shrink-0 text-[10px] px-1.5 py-0.5 rounded-full bg-bark-100 dark:bg-shadow-800 text-shadow-500 dark:text-shadow-400 border border-bark-200 dark:border-shadow-700">
                    {channelTypeTag(ch.channelId)}
                  </span>
                {/if}
              </div>
              {#if ch.displayLabel && ch.displayLabel !== ch.channelId}
                <p class="text-[10px] text-shadow-300 dark:text-shadow-600 mt-0.5 font-mono truncate">id: {ch.channelId}</p>
              {/if}
              <p class="text-[11px] text-shadow-400 dark:text-shadow-500 mt-0.5">
          ...
          {:else}
            <p class="p-4 text-sm text-shadow-400 dark:text-shadow-500 italic">No sessions found</p>
          {/each}
        </div>
      {/if}
    </div>
    ...
    <div class="flex-1 card-garden overflow-y-auto p-4">
      {#if !selectedChannel}
        <div class="flex items-center justify-center h-full text-shadow-300 dark:text-shadow-600 italic">
      ...
      {:else}
        <div class="space-y-6">
          ...
          <div class="pb-3 border-b border-bark-100 dark:border-shadow-800">
            <p class="text-sm text-shadow-500 dark:text-shadow-400">
              Channel: <span class="font-mono text-shadow-700 dark:text-bark-300">{selectedChannel}</span>
              <span class="text-shadow-300 dark:text-shadow-600 ml-2">({messages.length} messages)</span>
            </p>
          </div>
          ...
          {#if compactionAuditViews.length > 0 || selectedChannel}
            <div class="space-y-2">
              <h3 class="text-sm font-serif font-semibold text-shadow-600 dark:text-shadow-400">Compaction Audit</h3>
              <p class="text-[11px] text-shadow-400 dark:text-shadow-500">
                Click a summary to inspect source material hash metadata and JSONL verification.
              </p>

              {#if compactionAuditViews.length === 0}
                <p class="text-xs text-shadow-300 dark:text-shadow-600 italic py-2">
                    ...
                    <details class="group rounded-lg border border-bark-200 dark:border-shadow-700 overflow-hidden">
                      ...
                      <div class="px-4 py-3 space-y-2 bg-white dark:bg-shadow-900 border-t border-bark-100 dark:border-shadow-800">
                        ...
                        <p class="text-[11px] text-shadow-400 dark:text-shadow-500">
                          ...
                          {:else}
                            <span class="italic text-shadow-300 dark:text-shadow-600">not recorded</span>
                          {/if}
                        </p>
                        <p class="text-[11px] text-shadow-400 dark:text-shadow-500">
              ...
              <div class="flex gap-3">
                ...
                <div class="w-8 h-8 rounded-full shrink-0 flex items-center justify-center text-xs font-bold
                  {msg.role === 'user' ? 'bg-moss-100 text-moss-700 dark:bg-moss-900/30 dark:text-moss-300' :
                   ...
                   'bg-shadow-100 text-shadow-500 dark:bg-shadow-800 dark:text-shadow-400'}">
                  {msg.role === 'user' ? 'U' : msg.role === 'assistant' ? 'P' : 'S'}
                </div>

                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2 mb-1">
                    <span class="text-xs font-medium text-shadow-600 dark:text-shadow-400 capitalize">{msg.role}</span>
                    {#if msg.timestamp}
                      <span class="text-[11px] text-shadow-300 dark:text-shadow-600">{new Date(msg.timestamp).toLocaleString()}</span>
                    {/if}
                    {#if msg.originChannelId}
                      <span class="text-[10px] px-1.5 py-0.5 rounded-full bg-bark-100 dark:bg-shadow-800 text-shadow-400 dark:text-shadow-500 border border-bark-200 dark:border-shadow-700">
                  ...
                  {#if msg.toolCalls && msg.toolCalls.length > 0}
                    <details class="mt-2">
                      <summary class="text-xs text-shadow-400 dark:text-shadow-500 cursor-pointer hover:text-gold-600">
                        {msg.toolCalls.length} tool call{msg.toolCalls.length > 1 ? 's' : ''}
                      </summary>
                      <pre class="mt-1 text-[11px] font-mono bg-bark-50 dark:bg-shadow-900 p-2 rounded overflow-x-auto text-shadow-600 dark:text-shadow-400">{JSON.stringify(msg.toolCalls, null, 2)}</pre>
                    </details>
                  {/if}
                </div>
              </div>
            {:else}
              <p class="text-sm text-shadow-400 dark:text-shadow-500 italic text-center py-8">No messages in this session</p>
            {/each}
          </div>
        </div>
      {/if}
