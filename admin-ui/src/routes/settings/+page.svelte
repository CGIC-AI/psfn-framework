<script lang="ts">
  ...
  const INPUT_CLS = 'w-full px-3 py-2 rounded-lg border border-bark-300 bg-white text-shadow-900 text-sm focus:outline-none focus:ring-2 focus:ring-gold-300 focus:border-gold-400 transition-colors';
  const LABEL_CLS = 'block text-xs font-medium text-shadow-700 mb-1.5';
...
<div class="space-y-5">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
    ...
    <div class="flex items-center gap-3">
      ...
      <button onclick={doRefreshModels} disabled={refreshingModels}
        class="px-3 py-1.5 text-xs font-medium rounded-lg border border-bark-300
               text-shadow-700 hover:bg-bark-100
          ...
          <button
            ...
            class="px-3 py-1.5 text-xs font-medium capitalize transition-colors
              {mode === m
                ...
                : 'bg-white text-shadow-700 hover:bg-bark-100'}"
          >
            {m}
          </button>
        {/each}
  ...
  {#if loading}
    <div class="card-garden p-8">
      <div class="animate-pulse space-y-4">
        {#each Array(5) as _}
          <div class="h-10 bg-bark-200 rounded-lg"></div>
        {/each}
      </div>
    </div>
          ...
          <div>
            ...
            <div class="flex items-center gap-3">
              <input type="range" min="5" max="50" step="1" bind:value={memoryBudgetPct}
                class="flex-1 h-2 rounded-full appearance-none bg-bark-200
              ...
              <input type="number" min="5" max="50" bind:value={memoryBudgetPct}
                class="w-20 px-2 py-1.5 rounded-lg border border-bark-300 bg-white
                  text-shadow-900 text-sm text-center focus:outline-none focus:ring-2 focus:ring-gold-300" />
            </div>
      ...
      {#each ADVANCED_SECTIONS as section}
        ...
        {#if sectionKeys.length > 0}
          <div class="card-garden overflow-hidden">
            ...
            >
              <div class="flex items-center gap-3">
                <span class="flex items-center justify-center w-7 h-7 rounded-full bg-gold-50
                             text-gold-600 text-xs font-bold border border-gold-200">
                  {section.icon}
                </span>
            ...
            {#if openSections.has(section.id)}
              <div class="px-5 pb-5 space-y-3 border-t border-bark-100 pt-4">
                {#each sectionKeys as key}
                  {@const value = configValue(key)}
                  {@const ft = fieldType(value)}
                  <div class="flex flex-col sm:flex-row sm:items-center gap-2">
                    <label class="text-xs font-mono text-shadow-700 sm:w-60 shrink-0">{key}</label>
                    {#if ft === 'checkbox'}
                    ...
                    {:else if ft === 'number'}
                      <input type="number"
                        ...
                        class="flex-1 px-3 py-1.5 rounded-lg border border-bark-300
                               bg-white text-shadow-900
      ...
      {#if data}
        ...
        {#if otherKeys.length > 0}
          <div class="card-garden overflow-hidden">
            ...
            >
              <div class="flex items-center gap-3">
                <span class="flex items-center justify-center w-7 h-7 rounded-full bg-bark-100
                             text-shadow-700 text-xs font-bold border border-bark-300">
                  ?
                </span>
    ...
    <div class="space-y-4">
      ...
      {#if discoveredModels.length > 0}
        <div class="card-garden px-5 py-3 flex items-center justify-between">
          <span class="text-xs text-shadow-700">
            {discoveredModels.length} models discovered via proxy
          </span>
          <button onclick={doRefreshModels} disabled={refreshingModels}
            class="text-xs text-gold-600 hover:text-gold-700 font-medium disabled:opacity-50">
      ...
      {#each RAW_EDITORS as editor}
        ...
        <div class="card-garden overflow-hidden">
          <div class="flex items-center justify-between px-5 py-3 border-b border-bark-100">
            ...
            <div class="flex items-center gap-3">
              {#if status}
                <span class="text-xs font-medium {status.ok ? 'text-moss-600' : 'text-wilt-600'}">
                  {status.msg}
                </span>
              {/if}
              <button
          ...
          <textarea
            ...
            rows="14"
            class="w-full font-mono text-xs text-shadow-800
                   bg-bark-50 p-4
  ...
  {#if data?.env}
    <div class="card-garden px-5 py-4">
      <h2 class="text-xs font-serif font-semibold text-shadow-700 mb-2 uppercase tracking-wider">Environment</h2>
      <div class="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs text-shadow-700">
        {#if data.env.nodeVersion}
          <div>
            <span class="text-shadow-600">Node</span>
            <span class="font-mono ml-1 text-shadow-800">{data.env.nodeVersion}</span>
          </div>
        {/if}
        {#if data.env.platform}
          <div>
            <span class="text-shadow-600">Platform</span>
            <span class="font-mono ml-1 text-shadow-800">{data.env.platform}/{data.env.arch}</span>
          </div>
        {/if}
        {#if data.env.uptime !== undefined}
          <div>
            <span class="text-shadow-600">Uptime</span>
            <span class="ml-1 text-shadow-800">{Math.floor(data.env.uptime / 3600)}h {Math.floor((data.env.uptime % 3600) / 60)}m</span>
          </div>
        {/if}
        {#if data.env.memoryUsage}
          <div>
            <span class="text-shadow-600">Heap</span>
            <span class="ml-1 text-shadow-800">{(data.env.memoryUsage.heapUsed / 1_048_576).toFixed(0)}MB / {(data.env.memoryUsage.heapTotal / 1_048_576).toFixed(0)}MB</span>
          </div>
        {/if}
      </div>
    </div>
