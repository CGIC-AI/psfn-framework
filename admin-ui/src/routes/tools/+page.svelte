...
<div class="space-y-6">
  <div>
    <h1 class="font-serif text-2xl text-shadow-900 font-semibold">The Shed</h1>
    <p class="text-shadow-400 text-sm mt-1">Tool Registry</p>
  </div>
  ...
  {:else if error}
    <div class="card p-6 border-wilt-200">
      <p class="text-wilt-600 font-medium">Failed to load tools</p>
      <p class="text-shadow-400 text-sm mt-1">{error}</p>
    </div>
  {:else if data}
    <div class="card p-6">
      <p class="text-shadow-500 text-sm mb-4">
        Tool management is coming soon. Below is the current runtime state:
      </p>
