...
- `world.travel` and `world.body` are the companion-facing commands on the satellite socket. Both require enrolled device authority plus their own control capability (`world_travel`, `world_body`) in the device's server-owned `maxCapabilities`, so a browser client cannot grant itself either. `world.travel` re-resolves the place map for the destination before it touches the wire — an unmapped world is refused where the body still is, and no `placeId` is invented — and on arrival every later turn carries the new `placeId` and an arrival note. `world.body` submits one allowlisted locomotion action to the body runner and answers with submission, never completion. Both answer in the Hub's own refusal vocabulary; the door's prose never reaches a satellite
- `EIDOVERSE_BODY_*` bounds the Hub-owned body runner for allowlisted locomotion. `walk_to`, `face`, and `stop` are the complete allowlist; the runtime parser rejects every other verb, so `spawn` and `place` stay unreachable, and the runner exists only for a claim profile granting `avatar_action`. A walk blocks door-side until arrival or interruption, so it never runs on the turn that requested it: submission returns immediately and the outcome reaches a later turn as a content-free `eidoverse.body` context note. The door's own reply carries world coordinates and is never forwarded. `EIDOVERSE_BODY_WALK_TIMEOUT_MS` stays above the door's ~90s walk timeout; `EIDOVERSE_BODY_MAX_PENDING_NOTES` bounds the undrained outcome buffer
