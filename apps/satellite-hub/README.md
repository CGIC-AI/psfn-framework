...
- `EIDOVERSE_SNAPSHOT_*` enables optional first-person vision over the world's own `GET /snap` surface, which needs a live spectator renderer attached to that world. It is off unless `EIDOVERSE_SNAPSHOT_ENABLED=true` and the claim profile grants vision. The base URL is derived from `EIDOVERSE_MCP_WORLD_URL` (scheme swapped, `/ws` and query dropped) unless `EIDOVERSE_SNAPSHOT_BASE_URL` names a credential-free http/https origin. A frame lands on the same `visionCaptures`/`visionCaptureImages` seam Voxta uses, so the session transcript still receives text only. An absent renderer (503), an absent identity (404), a renderer timeout (504), a non-PNG body, a payload past `EIDOVERSE_SNAPSHOT_MAX_BYTES`, or an unreachable host each degrade to the text `look()` notes with one sanitized warning and no crash
