...
Purrsephone's autonomy system controls what she can do at runtime. Every tool
...
give Purrsephone increasing freedom as trust is established, and dial it back if
...
When Purrsephone spawns sub-agent shards via `spawn_shard`, the tools available
...
but inconvenient during development when Purrsephone needs to read reference
...
- **Local development**: When you want Purrsephone to read reference code,
  previous project implementations, or documentation outside her workspace.
- **Pair programming**: When working alongside Purrsephone on tasks that
