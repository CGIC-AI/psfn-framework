...
**Active concerns store** (PSFN-hme7): Lightweight SQLite-backed intention objects. "Check if the primary user ate today" (high priority, 48h TTL), "Follow up on project Thursday" (medium, 24h), "Monitor the primary user's energy level" (low, 8h). Injected into runtime context so PSFN sees her own intentions. Decay naturally. Influence memory retrieval as secondary query for relevance boosting.
