...
| Memory extraction (LLM) | No | No | **plugin-purrsephone only** | **Yes** (built-in module) |
| Memory dedup/merge | No | No | **plugin-purrsephone only** | **Yes** (LLM-based CSV merge) |
| Typed memory (episodic/semantic/emotional) | No | No | **plugin-purrsephone only** | No (flat facts only) |
| Salience decay | No | No | **plugin-purrsephone only** | No |
...
## Purrsephone Feature Requirements
...
### Memory Architecture Requirements (from PURRSEPHONE_MEMORY_ARCHITECTURE_v2.md)

| Tier | Description | Priority | Implemented Where |
|------|-------------|----------|-------------------|
| L0 | Append-only conversation archive | **Essential** | ElizaOS plugin-purrsephone |
| L1 | Working context (sliding window) | **Essential** | All platforms (session management) |
| L2 | Extracted typed memories (episodic, semantic, emotional, procedural, reflection) with decay | **Critical** | ElizaOS plugin-purrsephone |
...
| Memory extraction (L2) | **ElizaOS plugin-purrsephone** | Already built, typed, with decay -- port to new extension |
| Memory retrieval | **ElizaOS plugin-purrsephone** | Composite scoring (similarity x recency x emotion x importance x salience) |
