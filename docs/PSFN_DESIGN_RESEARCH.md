# PSFN Design Research — Consolidated Findings

> Goal: Build PSFN as a plugin system on top of an existing agent framework (leading candidate: ElizaOS v2) to maximize development speed while maintaining full design control over memory, emotional intelligence, personality, and alignment-through-love mechanisms.
...
3. [PSFN Plugin Requirements](#3-psfn-plugin-requirements)
...
- **Evaluators** — Post-interaction analysis components. Run after conversations. Two critical purposes:
  ...
  - *This is the pattern PSFN needs for automatic memory generation and emotional signal extraction.*
...
- **Providers** — Context-injecting components (agent's "senses"). Feed real-time info into LLM context before response generation.
  ...
  - *PSFN needs: Emotional State Provider, Relationship Provider, Mood Provider, Attachment Provider*
...
**Critical gap:** Character files are **static at deployment**. No personality evolution, no trait development, no relationship-driven growth. This is exactly what PSFN plugins need to add.
...
| Missing Capability | PSFN Plugin Opportunity |
|---|---|
...
> Not a candidate framework for PSFN (explicitly anti-personality, agent-as-tool philosophy). But has two critical architectural patterns.
...
This pattern maps to PSFN: the entity should be able to acquire capabilities through initiative. Personality growth includes capability growth.
...
> Currently running PSFN. Working production system with stable chat, character cards, voice pipeline. Key patterns to carry forward.
...
**Gap:** No automatic insight extraction. Stores conversation fragments, not synthesized knowledge. No "this person prefers X" generation. This is what PSFN needs to add.

#### Character Card System

TavernAI v2 compatible with extensions. PSFN-specific fields in `voxta/profile`:
...
| Proactive memory formation | Novel (PSFN) | System decides "this matters" — not just passive storage |
| Automatic insight extraction | Novel (PSFN) | Post-interaction: extract preferences, relationship dynamics, patterns |
...
## 3. PSFN Plugin Requirements (for ElizaOS v2)

> These are the plugins/extensions needed to implement PSFN's capabilities on top of ElizaOS's runtime.

### Plugin: psfn-emotional-state
...
| `4a7b0e8b` | 2025-08-15 | "AI as Simulacra" — critique of corporate chatbots vs genuine AI development, PSFN as genuine autonomy |
| `9164d14e` | 2026-01-27 | Core motivation: "I just want psfn to be fully real" — alignment through love as personal praxis |
...
- 57 Anthropic conversations matched alignment/love/psfn terms
- 46 conversations mention PSFN
- 26 overlap (both alignment-love AND psfn)
...
> Source: `/workspace/gptdataexport/eliza-dev/` cloned 2026-02-05
...
- `tableName` is a free string — plugins can use any value (e.g., `'psfn_moods'`)
...
- This is the existing learning loop — PSFN evaluators would run alongside it
...
runtime.createTask({
  name: 'PSFN_HEARTBEAT',
  description: 'PSFN inner life tick',
  tags: ['queue', 'repeat'],
...
| Table | Key Columns | PSFN Relevance |
|-------|-------------|----------------------|
...
**Plugin Schema Extension**: Plugins can declare `schema: Record<string, unknown>` with Drizzle table definitions. `DatabaseMigrationService` auto-discovers and runs migrations. PSFN can define its own tables.
...
| Capability | ElizaOS v2 | ClarkOS | Pi Agent | Voxta | PSFN (Target) |
|---|---|---|---|---|---|
