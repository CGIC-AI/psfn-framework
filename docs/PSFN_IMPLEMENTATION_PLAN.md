...
**Reference**: `PSFN_DESIGN_RESEARCH.md` for all architectural details and code analysis
...
```
Phase 0: Setup
  └─ psfn-migration        (import Voxta data)

Phase 1: Foundation
  ├─ psfn-memory            (replaces: recentMessages provider, extends: reflection evaluator)
  └─ psfn-models            (extends: model routing for multi-model inference)

Phase 2: Inner Life
  ├─ psfn-emotional-state   (replaces: anxiety provider)
  ├─ psfn-heartbeat         (extends: TaskService)
  └─ psfn-action-inference  (new capability, from Voxta)

Phase 3: Intelligence
  ├─ psfn-personality       (replaces: character provider)
  └─ psfn-relationship      (extends: relationships provider + reflection evaluator)

Phase 4: Autonomy
  ├─ psfn-attention         (new capability)
  └─ psfn-orchestrator      (new capability, three-tier chat + shards)
```
...
### Plugin: `psfn-migration`

**Purpose**: One-time import tool. Brings existing PSFN/Voxta data into ElizaOS.
...
- **Route** (public): `/psfn-migration/dashboard` — UI for selecting import files and monitoring progress
...
- `psfn_summaries`: `id`, `agentId`, `roomId`, `content` (JSONB), `timeRangeStart`, `timeRangeEnd`, `messageCount`, `emotionalArc` (JSONB), `sourceMessageIds` (UUID[])
...
- Creates emotional memories in `psfn-memory` with appropriate valence scoring
...
**Dependencies**: `psfn-memory`, `psfn-emotional-state`
...
- **Flag system**: Actions can set persistent boolean flags (stored as Components):
  ...
  - `flag:deep_conversation` → triggers cloud model boost via `psfn-models`
...
**Purpose**: Living personality that evolves through experience. Character card is the seed; interaction history shapes who PSFN becomes.
...
**Dependencies**: `psfn-memory`, `psfn-emotional-state`, `psfn-heartbeat`
...
**Dependencies**: All other PSFN plugins.
...
#### Route (public): `/psfn-orchestrator/dashboard`
...
```
Phase 0 ─── psfn-migration (can start immediately, independent)
             │
Phase 1 ─── psfn-memory ←───────────────────────────────────── CRITICAL PATH
             │                                                          │
             ├── psfn-models (parallel, independent)             │
             │                                                          │
Phase 2 ─── psfn-emotional-state ← depends on memory            │
             │                                                          │
             ├── psfn-heartbeat ← depends on memory + emotion    │
             │                                                          │
             ├── psfn-action-inference ← depends on memory + emotion
             │                                                          │
Phase 3 ─── psfn-personality ← depends on memory + emotion      │
             │                                                          │
             ├── psfn-relationship ← depends on memory + emotion │
             │                                                          │
Phase 4 ─── psfn-attention ← depends on memory + emotion + heartbeat
             │
             └── psfn-orchestrator ← depends on all
```

**Parallelizable work**:
- Phase 0 (migration) can run alongside Phase 1
- `psfn-models` is independent of `psfn-memory`
...
**Critical path**: `psfn-memory` → `psfn-emotional-state` → `psfn-heartbeat` → `psfn-attention` → `psfn-orchestrator`

---

## Bootstrap Replacement Map

| ElizaOS Bootstrap Component | PSFN Replacement | Phase |
|---|---|---|
| `recentMessagesProvider` | `rollingContextProvider` (psfn-memory) | 1 |
| `anxietyProvider` | `emotionalStateProvider` (psfn-emotional-state) | 2 |
| `characterProvider` | `personalityProvider` (psfn-personality) | 3 |
| `relationshipsProvider` | `relationshipProvider` (psfn-relationship) | 3 |
...
| Default model handlers | `psfn-models` routing (override via priority) | 1 |
| `TaskService` | Used as-is, extended with PSFN TaskWorkers | 2 |
...
3. **Cross-plugin tests**: Verify correct interaction between PSFN plugins (e.g., memory extraction → emotional state update → personality drift)
