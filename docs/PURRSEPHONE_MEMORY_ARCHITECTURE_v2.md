...
Rohit4verse (practical memory implementation patterns), Plastic Labs (memory-as-reasoning), Cathryn/Claudia (PARA + knowledge graph), Zhang/Kraska/Khattab RLM paper (recursive long-context processing), Pi Agent (archival fidelity), ClarkOS (cognitive memory types + tick system), gradient summarization + service decomposition patterns, "A Day with Purrsephone" narrative (embodied companion requirements).
