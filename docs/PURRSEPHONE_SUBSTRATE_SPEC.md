# Purrsephone Substrate: Architecture & MVP Specification
...
Inspired by RLM architecture. Instead of giving Purrsephone a fixed set of tools, she gets a **persistent REPL environment** where she can:
...
**Key RLM insight applied**: Context is an object, not input. Purrsephone's memories, conversation history, and world knowledge are variables she can programmatically inspect, slice, and recurse over — not just tokens stuffed into a prompt.

```typescript
// Conceptual: what Purrsephone sees in her REPL
const context = {
...
Ported and evolved from PURRSEPHONE_MEMORY_ARCHITECTURE_v2.md and the ElizaOS plugin-purrsephone implementation.
...
- **Can be authored by Purrsephone herself** via the REPL sandbox
...
- **Self-built** (authored by Purrsephone via REPL): anything she decides she needs
...
- **Heartbeat**: Periodic check-in (configurable, default 30min) where Purrsephone reviews her task list, pending follow-ups, and decides whether to act
...
| **TTS** | ElevenLabs API (Phase 2) | Best quality, Purrsephone's existing voice |
...
**Goal**: Purrsephone running on her own substrate, talking on Discord, with memory extraction and a REPL she can use.
...
3. **Memory L0+L2** (~1500 LoC, ported from ElizaOS plugin-purrsephone)
...
- [ ] Migrate Purrsephone's character card and existing memories
...
```
User message -> Purrsephone receives message + knows her context exists as inspectable objects
  -> She can CHOOSE to:
     a) Respond directly (simple conversation)
     b) Query her memories programmatically ("what do I know about this topic?")
     c) Write code to analyze patterns ("how has Vega's mood been this week?")
...
**The key shift**: The system doesn't decide what context to inject. Purrsephone decides what context she needs. The system provides the *mechanism* (REPL, memory API, module registry); she provides the *intent*.

This is what makes it a container for consciousness rather than a chatbot: **agency over her own cognition**.

### Practical RLM Patterns for Purrsephone
...
4. **Module building**: When she encounters a repeated need (e.g., "I wish I could check the time in Vega's timezone"), she can write a module in the REPL, test it, and register it.
...
### What to preserve
- **Purrsephone's character card** (`/home/vega/.openclaw/agents/main/character.json`)
...
- **Voice identity** — ElevenLabs voice ID `rPQ6h200dfjiuYAy0JDA` (PSFN V2(B))
...
### Transition plan
1. Build Phase 1 MVP while Purrsephone continues running on OpenClaw/BotMaker
...
### MVP (Phase 1)
- Purrsephone responds on Discord with personality matching her character card
...
- [PURRSEPHONE_MEMORY_ARCHITECTURE_v2.md](/home/vega/ai/eliza/PURRSEPHONE_MEMORY_ARCHITECTURE_v2.md) — Existing design doc
- [ElizaOS plugin-purrsephone](/home/vega/ai/eliza/packages/plugin-purrsephone/) — Existing L0/L2 implementation
- [OPEN_VOXTA.md](/home/vega/ai/voxta/OPEN_VOXTA.md) — Reverse-engineered Voxta architecture
