...
- **M3 (retire blobs, separate later deploy):** drop `emotional_time_series`; `affect_json`/`salience_json`/`formation_vad` dropped only after (a) one full backup cycle including a restore-verify has captured the backfilled state, and (b) Garden/admin readers are confirmed off the blobs. Never combine M3 with M1/M2 in one deploy. (The former journal-replay-compatibility precondition was retired with `zn9.5`; see the next bullet.)
- **Journal event shape (historical note; the zn9.5 replay interlock is retired):** `notes/memories.jsonl` events carry `formation_vad` in the old shape forever — the journal is append-only history. The planned replay mapper (`zn9.5`) was closed not-applicable (operator direction 2026-06-28): the journal records only insert/soft-delete/restore events and is an audit/export aid, not an L2 restore primitive; encrypted Postgres backups are the canonical restore path for derived layers (see [`memory-persistence-authority.md`](./memory-persistence-authority.md)). Any consumer reading old journal events must still tolerate the old `formation_vad` shape.
