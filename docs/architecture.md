...
- starts Discord, Telegram, and installable channel plugins when enabled, each isolated by `ChannelSurfaceSupervisor` (`src/channels/backplane/channel-isolation.ts`): a surface whose load, init, start, or stop fails is degraded (bounded background retry for retryable network/5xx errors under the env-owned `CHANNEL_SURFACE_START_RETRY_BASE_DELAY_MS` / `_MAX_DELAY_MS` / `_MAX_ATTEMPTS` policy, `src/boundary/gateway/channel-start-retry.ts`; the retired `DISCORD_START_RETRY_*` names are refused at startup) or disabled on its own, reported as `channel_surface_failed` / `channel_surface_disabled` health events, and never stops the gateway or another channel, plus the voice module host (`GatewayVoiceModuleHost` with the Discord reverse-RPC voice module). Gateway-hosted Wyoming is retired: `createGatewayVoiceSurfaces` throws at startup if `WYOMING_ENABLED` is set, and Wyoming/OpenHome endpoints now run through the Satellite Hub — see [`channels/voice.md`](channels/voice.md) and [`apps/satellite-hub.md`](apps/satellite-hub.md);
...
1. Resolves the startup context, alerts configuration, and lifecycle runtime contract, then runs `enforceNetworkIsolationOnStartup`, a three-way egress proof (`src/app/agent/network-isolation-probe.ts`). It probes several independent IP-literal HTTP/HTTPS targets in parallel: any answer (including an intercepting proxy's block page or TLS handshake) is **reachable** and fatal; all targets network-blocked plus a launcher-declared mechanism in `PSFN_AGENT_EGRESS_ISOLATION` (`kubernetes-network-policy` from the Helm chart when it renders the agent NetworkPolicy, `container-internal-network` from Compose) is **isolated**; anything else (no declaration, DNS or unclassified failures) is **unknown** and fails startup closed. Failed probes alone never prove isolation. The only bypass is the explicit, logged `ALLOW_AGENT_OUTBOUND_NETWORK=true` override.
