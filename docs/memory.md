...
- **Affect authorship (h4fp.6).** Episodes are born **affect-empty** (`affect: { labels: [] }`). Deterministic synthesis and its keyword/VAD heuristics must never write a first-person felt-affect claim: those machine signals move to a clearly machine-labeled `machineSignals` sidecar (`{ source, topicTags, vad? }`) as topic tags and retrieval hints, never her feelings. The companion's own felt sense of an episode is authored only by her, in prose, in the `meaning` field via the nightly dream pass — no machine path writes `affect` or `meaning`. Merge/consolidation/refinement paths preserve affect and never fabricate it (an affect-empty candidate never gains a machine-derived `arousal: 0`). The `machineSignals` sidecar is version-gated: it is permitted only at `EPISODIC_CONTRACT_VERSION >= 2`. The parser accepts stored v1 and v2 rows and preserves the stored version verbatim (no silent upgrade); forward-writing is v2, and migration of historical rows is a separate concern (h4fp.7). In companion-facing retrieval (the always-on landmark block and the episode drill-down) an episode without her authored meaning renders with an explicit **unreviewed** marker — a machine-drafted summary never reads as her settled lived past. The dream pass reviews under a **prioritized nightly budget**: episodes rank by highest participant contact-trust first (contact store `trustOrd`), then machine-signal density (topic-tag count plus VAD presence), then oldest-first — deterministic and logged per pass, so the capped review lands on likely-mattering episodes.
