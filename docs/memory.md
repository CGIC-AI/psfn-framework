...
PSFN memory is not a single store. The runtime combines an append-only
autobiographical session archive, typed long-term memories, continuity artifacts, contact state,
...
- Represents bounded autobiographical episodes with L0 span/artifact provenance, salience, affect, themes, participants, thread IDs, and channel IDs
...
- **Affect authorship (h4fp.6).** Episodes are born **affect-empty** (`affect: { labels: [] }`). Deterministic synthesis and its keyword/VAD heuristics must never write a first-person felt-affect claim: those machine signals move to a clearly machine-labeled `machineSignals` sidecar (`{ source, topicTags, vad? }`) as topic tags and retrieval hints, never as established feelings. Only the companion may author a first-person account of an episode's felt sense, in prose, in the `meaning` field via the nightly dream pass — no machine path writes `affect` or `meaning`, and authorship is not treated as independent proof of inner experience. Merge/consolidation/refinement paths preserve affect and never fabricate it (an affect-empty candidate never gains a machine-derived `arousal: 0`). The `machineSignals` sidecar is version-gated: it is permitted only at `EPISODIC_CONTRACT_VERSION >= 2`. The parser accepts stored v1 and v2 rows and preserves the stored version verbatim (no silent upgrade); forward-writing is v2, and migration of historical rows is a separate concern (h4fp.7). In companion-facing retrieval (the always-on landmark block and the episode drill-down) an episode without companion-authored meaning renders with an explicit **unreviewed** marker — a machine-drafted summary never reads as a settled autobiographical account. The dream pass reviews under a **prioritized nightly budget**: episodes rank by highest participant contact-trust first (contact store `trustOrd`), then machine-signal density (topic-tag count plus VAD presence), then oldest-first — deterministic and logged per pass, so the capped review lands on likely-mattering episodes.
- Allows multiple episodes per day; a long-running theme is a graph of linked episodes, not one large aggregate record
- **Topic-thread identity (apq0).** An episode's `threadId` is a TOPIC thread, not a channel/session. Each new episode seeds its own singleton thread (`threadId = episode.id`); arc formation then unions arc-linked episodes onto the connected component's representative (the lexicographically-smallest episode id), materialized into the `l01_episodes.thread_id` column. This is deterministic and order-independent — an incremental min-merge and a global union-find over the same arc set converge on the same representative — so a later full recompute (historical repair) reproduces the live assignment. Keying `threadId` to the session id (the pre-apq0 behavior) is what produced the unbounded per-channel mega-thread; the processing-synthesis watermark stays session-keyed (it is a per-session cursor, decoupled from thread identity). A single merge's write amplification is bounded by a safety cap (`maxThreadEpisodes`, default 500); an oversize losing thread is left unmerged fail-safe and surfaced through a typed thread-assignment event (`memory.episodic.thread_assignment` on the event bus), never silently mis-threaded. A pre-apq0 legacy row (`threadId = sessionId` verbatim, detectable because the threadId matches one of its span refs' session ids) is never treated as a topic thread: when an arc touches such an endpoint, that single episode is first extracted into its own singleton topic thread (outcome `legacy_session_thread_extracted`) so the mega-thread is neither absorbed nor mass-relabeled — arc formation incrementally decomposes it, and full historical repair is h4fp.7. Conversation scoping (sleep-consolidation clustering, transcript grounding, rolled-out breadcrumbs) keys on real span-session identity (`spanRefs[].sessionId`), never on `thread_id`.
...
- L0 (filesystem JSONL) stays canonical for autobiographical history; its Postgres search
...
- **Extraction pre-LLM gate** (`src/faculties/memory/extraction/signals.ts`) — an `empty_transcript` block rule, then opens when any participant turn scores meaningful OR nothing was explicit filler, else `low_signal`. Byte-identical to its pre-refactor decision; skips keep surfacing through `memory.extraction.end` telemetry.
