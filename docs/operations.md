...
- With `verifyRestore` enabled, every scheduled cycle restores the dump into a dedicated scratch database (`<dbname>_restore_verify`, derived from the runtime database URL — create it once, owned by the runtime role; its `public` schema is wiped each run) and asserts schema, pgvector functionality on restored vectors, critical-table presence, and that tables populated at the source restored non-empty. The dump archive table of contents is also checked via `pg_restore --list`, the companion-tree manifest hashes are re-verified, and the L0 journal snapshot must parse as JSONL.
