...
| `{{runtime_current_message_author_xml}}` | turn | `runtime-context:buildDynamicPromptTemplateVariables` | Preformatted current message author XML. Group turns may include timezone/local_time and content-free last_direct_interaction_* attributes from another private chat when known; private content and channel identity are never included. |
...
| `{{runtime_recent_active_participants_xml}}` | turn | `runtime-context:buildDynamicPromptTemplateVariables` | Compact recent active participant XML for group turns, capped at five deduped authors. Known human participants may carry content-free last_direct_interaction_* attributes from another private chat; private content and channel identity are never included. |
