...
- **tooling** — `bd`/`rg`/`psql` availability via PATH probing, plus the beads tools enabled/disabled determination (`BEADS_TOOLS_ENABLED=false` always disables; otherwise the tool is registered only when a Beads database is provisioned — `BEADS_DIR` when set, else a `.beads` directory at the workspace or repo root — so `BEADS_TOOLS_ENABLED=true` without a database fails closed at registration instead of advertising a tool whose every call fails, bead `psfn-framework-povuo`);
...
The gateway `web.fetch` method enforces the policy **twice**: synchronously in the policy gate (`evaluatePolicy`, so the audit log reflects the real decision) and again in `fetchWithPolicyChecks` before connecting. `fetchWithValidatedRedirectChain` re-policies **every redirect hop** with the same lane/config, bounds hops by `maxRedirectHops` (default 5, bounded to [0, 20]), detects redirect loops via a visited set, and **strips `authorization`/`cookie`/`proxy-authorization` headers permanently once a redirect leaves the original request origin** (scheme, host, or effective port). One additional lane exists: `discovery` (requires a normalized URL allowlist; it still always-blocks cloud metadata/link-local). Internal-network fetches use the default lane with `webFetchAllowInternalNetwork` plus the allowlists. The `home_assistant` lane is declared in the lane union; Home Assistant traffic rides the default lane with port-pinned host allowlist entries (e.g. `ha.local:8123`).
