...
Tracked as child beads of epic `psfn-framework-65rk`: `65rk.1` harness port (shared probe lib, fail-closed env, tier sweep, scorecard coverage cross-check), `65rk.2` one-command fresh bootstrap with Artie, `65rk.3` S10 case authoring for the appendix, `65rk.4` support-companion fixtures. Until they land, rounds run on the Sprint-9 script set at `/mnt/c/Temp/PSFN-TEST/sprint9-shakedown/artifacts/` **with the env sourced first** (its fallback defaults are stale) — treat that as bridge tooling only.
