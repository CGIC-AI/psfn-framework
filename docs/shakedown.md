...
## Cluster policy (standing)

Shakedown, e2e, eval, rollout-smoke, and any other testing **never** run on
the production cluster (`<prod-kube-context>`, namespace `<prod-namespace>`). Testers
kept using it and leaving unclean state. Production is live companion home
only — no RC ships, no PVC wipes, no harness traffic, no "quick kube check."

The **only** Kubernetes target for those activities is the test cluster
(`k3d-psfn-kube`, namespace `<round-namespace>`), or a newly created disposable
cluster. Abort if the kube context is `<prod-kube-context>` or the namespace
is `<prod-namespace>`.
...
production companions to "run the kube lane."

## 2026-08-29 focused recertification (`qjsj6`)

Do **not** run the historical multi-hour full matrix. This round is two
bounded passes plus the soak criteria already tracked by
...
| Bead | Pass | Assert |
| --- | --- | --- |
| `qjsj6` | both | Rebuild/deploy the fixed candidate; 3–4h automation soak: connection ceilings, zero sibling-role denials, queue progress, proactivity, automation |
| `woowa` | 1 | Autonomous required-tool calls emit real args or fail over to Kimi; never one chat-capable candidate |
| `e6938` | both | No PostgreSQL connection-slot exhaustion or agent restart from startup connection pressure |
| `wzqs5` | 2 | Zero sibling-role denials (`SET ROLE` / Garden tenant-role) for the test companions |
| `yqv0w.4` | 1 | Defer-until-idle queues progress; no permanent valid-work drop |
| `y0bft` | 1 | Episodic/sleeptime drains resume; no 5-minute fleet-leader churn |
| `xjiy1` / `2dn37.4` | 2 | Per-companion EmoSim posture; no primary fallback |
| `k1tpa` | 1 | Operator alerts through configured sinks; unconfigured sink is typed, not a crash |

Out of scope: Biography / Automata Bus feature lanes, production
`<prod-kube-context>` mutation, OpenRouter chat spend.
...
The kube lane is **test cluster only**. Confirm
`kubectl config current-context` is `k3d-psfn-kube` (or a newly created
disposable cluster) and the namespace is `<round-namespace>`. Stop if the context is
`<prod-kube-context>` or the namespace is `<prod-namespace>`.
...
3. Ship the RC to the test cluster: `npm run ship:kube`
   (`scripts/ops/ship-kube-update.sh`, component-selective, contract-hash
   guarded). Point it at kube-test, never kube-main.
4. Gate with `bash scripts/ops/validate-kube-rollout.sh --smoke` (rollout
   status for agent/gateway/garden, garden health, `/v1/models` companion
   route, two-turn chat smoke).
5. Open the round by telling the **copies** they are the test instances and
   which sprint build they are on. Do not treat test PVCs as production
   continuity; reset them when the round requires a clean fleet.
