...
**postgres** is an optional in-cluster database with pgvector for vector similarity search. Can be replaced with an external managed database.
...
```bash
# From the repo root:
docker build -f docker/Dockerfile.gateway -t psfn-gateway:latest .
docker build -f docker/Dockerfile.agent -t psfn-agent:latest .
```
...
```bash
docker tag psfn-gateway:latest registry.example.com/psfn-gateway:v1.0.0
docker tag psfn-agent:latest registry.example.com/psfn-agent:v1.0.0
docker push registry.example.com/psfn-gateway:v1.0.0
docker push registry.example.com/psfn-agent:v1.0.0
```
...
| `DATABASE_PATH` | `/app/companion-data/psfn.db` | SQLite file path (when DB_PROVIDER=sqlite) |
...
| `DATABASE_URL` | When DB_PROVIDER=postgres | Postgres connection string |
...
1. Set the connection string in `secrets.yaml`:
   ```yaml
   stringData:
     DATABASE_URL: "postgres://user:password@your-rds-host.amazonaws.com:5432/psfn"
   ```

2. Set `DB_PROVIDER` to `postgres` in `configmap.yaml`:
   ```yaml
   data:
     DB_PROVIDER: "postgres"
   ```

3. Edit `k8s/overlays/production/kustomization.yaml` — uncomment the `patchesStrategicMerge` section and the StatefulSet delete patch to remove the in-cluster Postgres.

4. Set `externalName` in `k8s/overlays/production/external-postgres.yaml` to your database hostname so the `postgres` service DNS resolves to the external host:
   ```yaml
   spec:
     externalName: your-rds-host.amazonaws.com
   ```

5. Ensure your external database has the `vector` and `pg_trgm` extensions installed. Migrations run automatically on startup.
