...
export function resolvePullRequestMetadata({
  ...
  if (env.CHANGE_BUDGET_USE_OFFLINE === 'true') {
    if (!offline) {
      throw new Error('CHANGE_BUDGET_USE_OFFLINE requires explicit change-budget metadata');
    }
    return { ...offline, source: 'publisher-provided metadata' };
  }
