...
test('uses publisher-provided metadata without querying a not-yet-created PR', () => {
  const metadata = resolvePullRequestMetadata({
    options: { exception: true },
    env: {
      CHANGE_BUDGET_EXCEPTION: 'true',
      CHANGE_BUDGET_PR_BODY:
        '## Change-budget exception\nThis coherent recovery is intentionally oversized.',
      CHANGE_BUDGET_USE_OFFLINE: 'true',
    },
    runGh() {
      throw new Error('publisher metadata must avoid GitHub lookup');
    },
  });

  assert.equal(metadata.source, 'publisher-provided metadata');
  assert.equal(metadata.exception, true);
});
