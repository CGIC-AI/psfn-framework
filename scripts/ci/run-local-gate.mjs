...
export function validateStateAttestation(attestation, state, options) {
  ...
  const activePlan = plan.filter(({ skip }) => !skip);
  const gates = activePlan.map(({ name }) => name);
  const gatePlan = activePlan.map(gateCommandString);
  return {
    plan,
    gates,
    gatePlan,
    result: validateAttestation(attestation, { ...state, gates, gatePlan }),
  };
}

export function gateEnvironment(gate) {
  const env = { ...process.env };
  if (gate.name === 'change-budget' && env.CHANGE_BUDGET_EXCEPTION === undefined) {
    env.CHANGE_BUDGET_EXCEPTION = 'false';
  }
...
export async function runLocalGate({
  ...
  const { plan, gates, gatePlan } = validation;
  if (planOnly) {
  ...
  // settings/supply-chain contracts — runs before any heavy suite so
