...
export function firstChatGapNotes(input: {
  ...
  switch (input.mode) {
    ...
    case 'local':
      lines.push(
        '  - Local dev does NOT provide Postgres. Provision the companion_main_runtime and '
        + 'shared_schema_migration roles, then set POSTGRES_DATABASE_URL, '
        + 'COMPANION_MAIN_DATABASE_URL, and SHARED_SCHEMA_MIGRATION_DATABASE_URL before booting; '
        + 'the runtime persistence layer is Postgres-only.',
      );
      break;
    case 'kubernetes':
      lines.push(
...
export function buildEnvEntries(input: {
  ...
  if (input.mode === 'local') {
    ...
    entries.push({
      envName: 'PSFN_RUNTIME_ROOT',
      value: resolveOnboardingRuntimeRoot({ roots: input.roots }),
      comment: 'Canonical runtime root for companions.json relative paths',
    });
  }
