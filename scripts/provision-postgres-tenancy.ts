...
async function main(): Promise<void> {
  ...
  try {
    ...
    // psfn-framework-h248l.7 retired the cross-schema welfare-verifier read
    // grant: a fleet gateway now asks each companion's own runtime authority
    // (`welfare.grant.verify`) instead of reading a sibling's
    // agent_background_work_jobs, so no fleet-wide reader role exists to
    // provision and no schema outside its own tenant is readable from here.
    for (const plan of plans) {
