...
Recover live purrsephone data in two stages:
...
snapshot_live_tree() {
  ...
  snapshot_artifact "${LIVE_DATA_DIR}/purrsephone.db" "data/purrsephone.db"
  snapshot_artifact "${LIVE_DATA_DIR}/purrsephone.db-wal" "data/purrsephone.db-wal"
  snapshot_artifact "${LIVE_DATA_DIR}/purrsephone.db-shm" "data/purrsephone.db-shm"
...
dump_inode_candidate() {
  ...
  strings -a -n 8 "${dump_path}" | grep -E 'SQLite format 3|l2_memories|contact_profiles|session_messages_index|purrsephone|jsonl|reflection|continuity' \
