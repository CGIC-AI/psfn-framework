...
assert_workspace_path_source() {
  [ -f "${ENV_SOURCE}" ] || fail "Source env must define a non-empty WORKSPACE_PATH: ${ENV_SOURCE}"

  local workspace_entries
  workspace_entries="$(grep -E '^(export[[:space:]]+)?WORKSPACE_PATH=' "${ENV_SOURCE}" || true)"
  WORKSPACE_PATH_ENTRY="${workspace_entries##*$'\n'}"

  if [[ "${WORKSPACE_PATH_ENTRY}" == export[[:space:]]* ]]; then
    WORKSPACE_PATH_ENTRY="${WORKSPACE_PATH_ENTRY#export}"
    WORKSPACE_PATH_ENTRY="${WORKSPACE_PATH_ENTRY#"${WORKSPACE_PATH_ENTRY%%[![:space:]]*}"}"
  fi

  local workspace_path="${WORKSPACE_PATH_ENTRY#*=}"
  workspace_path="${workspace_path#"${workspace_path%%[![:space:]]*}"}"
  workspace_path="${workspace_path%"${workspace_path##*[![:space:]]}"}"

  [ -n "${WORKSPACE_PATH_ENTRY}" ] \
    && [ -n "${workspace_path}" ] \
    && [ "${workspace_path}" != '""' ] \
    && [ "${workspace_path}" != "''" ] \
    && [[ "${workspace_path}" != \#* ]] \
    || fail "Source env must define a non-empty WORKSPACE_PATH: ${ENV_SOURCE}"
}
