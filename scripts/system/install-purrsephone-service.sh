...
UNIT_TEMPLATE_PATH="${SCRIPT_DIR}/purrsephone.service.template"

SERVICE_USER="purrsephone"
SERVICE_GROUP="purrsephone"
SERVICE_HOME="/var/lib/purrsephone"
APP_ROOT="/var/lib/purrsephone/app"
RUNTIME_ROOT="/var/lib/purrsephone/runtime"
ENV_FILE="/etc/purrsephone/purrsephone.env"
UNIT_FILE="/etc/systemd/system/purrsephone.service"
...
Provision a dedicated purrsephone system-account deployment with:
- a service-owned app checkout
- a bundled node binary outside vega's home
...
log() {
  printf '[purrsephone-install] %s\n' "$*"
}

fail() {
  printf '[purrsephone-install] error: %s\n' "$*" >&2
  exit 1
}
...
resolve_paths() {
  ...
  SERVICE_HOME="$(stage_default_path "${SERVICE_HOME}" "/var/lib/purrsephone")"
  APP_ROOT="$(stage_default_path "${APP_ROOT}" "/var/lib/purrsephone/app")"
  RUNTIME_ROOT="$(stage_default_path "${RUNTIME_ROOT}" "/var/lib/purrsephone/runtime")"
  ENV_FILE="$(stage_default_path "${ENV_FILE}" "/etc/purrsephone/purrsephone.env")"
  UNIT_FILE="$(stage_default_path "${UNIT_FILE}" "/etc/systemd/system/purrsephone.service")"
}
