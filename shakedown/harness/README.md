...
| `PSFN_ADMIN_BASE` | Garden admin base (local target only) | omit for kube; kube admin and readiness traffic uses the unified gateway origin |
| `TESTING_HARNESS_API_KEY` | dedicated gateway credential for the persistent `api:testing-harness` room | required for ordinary conversational harness traffic; satellite-claim cases use the scoped bearer below |
...
| `PSFN_SHAKEDOWN_IMAGE_PROVIDER` | image provider for `image_create` / `image_edit` / `selfie_create`: `settings` (omit provider; the deployment's `imageProvider` selects), or `auto` / `fal` / `comfyui` / `comfyui_mcp` / `openrouter` | required whenever an image case is selected, no default; an explicit provider other than `auto` must appear in a successful tool result |
...
between tiers. **kube** targets the test companion's persistent deployment: the sweep flips
...
| `bootstrap-local.mjs` | One-command local bootstrap: validates protected roots before any write, builds the RC, seeds owner files, imports the reference companion, launches split runtime, and prints the exact persisted first-turn record. |
...
the corresponding `PSFN_SHAKEDOWN_*_PLACE_*` values in the companion env template.
The satellite claim cases additionally require
`PSFN_SHAKEDOWN_PHYSICAL_SATELLITE_API_KEY`, set to the enrolled bearer whose
derived principal ID is authorized for all three synthetic fixture endpoints.
The satellite CogSec document case reuses that same scoped bearer.
...
token to that enrolled device, companion, place, and satellite session. The
signing key must be an untracked mode-0600 file whose public key matches the
active Hub verifier key. The token is attached only to the live request and is
never copied into a request summary, diagnostic, or proof artifact.
The local single-companion setup preserves the generated matching key at
`$PSFN_SHAKEDOWN_HOST_ROOT/hub-device-assertion-private.pem` (default
`$HOME/psfn-shakedown`) and verifies it against the seeded owner file.
...
`shakedown/companion/shakedown.env.template`). The bootstrap host must have `psql`
...
# kube tier sweep — flip via the unified-origin settings API, no PVC edit,
# direct Garden port-forward, or redeploy.
PSFN_TARGET=kube \
...
3-tier probe that was run against the primary test companion on the k3d
...
Kube context and namespace are round-specific (example: context `k3d-psfn-kube`,
namespace `<round-namespace>`). Fetch the tokens from the app secret (names
only — never paste the values into any committed file):

```bash
kubectl config use-context k3d-psfn-kube
export ADMIN_TOKEN="$(kubectl -n <round-namespace> get secret psfn-app -o jsonpath='{.data.ADMIN_TOKEN}' | base64 -d)"
export TESTING_HARNESS_API_KEY="$(kubectl -n <round-namespace> get secret psfn-app -o jsonpath='{.data.TESTING_HARNESS_API_KEY}' | base64 -d)"
```

Stand up the two port-forwards (Garden admin :10054, gateway :10053):

```bash
kubectl -n <round-namespace> port-forward svc/psfn-garden 10054:10054 &
kubectl -n <round-namespace> port-forward svc/psfn-gateway 10053:10053 &
```
...
```bash
# inspect first
kubectl -n <round-namespace> exec deploy/<agent-deploy> -- cat <system-data-mount>/capability-tier.json
