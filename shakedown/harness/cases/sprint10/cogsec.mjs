...
function buildCogSecCase(ctx, services, env, {
  ...
  return {
    ...
    execute: async ({ sessionId, apiUserId, signal }) => {
      ...
      const assertionHeaders = requireSatellitePrefix
        ? issueHubDeviceAssertionHeaders({
            services,
            env,
            prefix: requireSatellitePrefix,
            caseId: id,
          })
        : {};
      const response = await postChatCompletion({
        ...
        headers: buildChatHeaders({
          ...
          sessionId,
          privacy: 'private',
          extra: { ...headers, ...assertionHeaders },
        }),
        message,
        content: [
      ...
      const turnRecord = await services.waitForTurnRecord({
        ...
        timeoutMs: 120_000,
        searchAllChannels: Boolean(requireSatellitePrefix),
        signal,
      });
