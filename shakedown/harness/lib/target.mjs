...
// canonical gateway HTTPS origin. Fleet admin APIs and the kube readiness proof
// use the gateway unified origin; nothing here ever names a namespace, service,
// or /mnt path.
//
...
export function resolveTarget(env = process.env) {
  ...
  const directAdminBaseUrl = isKube
    ? null
    : requireEnv('PSFN_ADMIN_BASE', 'Garden admin base URL', env);
  ...
  return {
    ...
    adminReadinessUrl: isKube
      ? `${adminBaseUrl}/api/admin/tools/adaptive`
      : `${directAdminBaseUrl}/health`,
    apiKey,
    adminToken: isKube
      ? apiKey
  ...
  switch (command) {
    case 'resolve': {
      ...
      process.stdout.write(`${JSON.stringify({
        ...
        adminReadinessUrl: contract.adminReadinessUrl,
