...
const CONFIG = (() => {
  try {
    ...
    // target uses the gateway unified origin for fleet Garden APIs and for the
    // kube readiness proof. The hardened Garden listener is mTLS-only.
...
const ADMIN_READINESS_URL = CONFIG.adminReadinessUrl;
...
const MANAGED_SKILLS_ROOT = optionalEnv('PSFN_MANAGED_SKILLS_ROOT') ?? `${CONFIG.workspacePath}/skills`;
const BEADS_DIR = optionalEnv('PSFN_BEADS_DIR');
...
const OPERATOR_APPROVAL_TARGET = (() => {
  try {
...
let hubDeviceAssertionIssuer = null;

function issueHubDeviceAssertion(input) {
  hubDeviceAssertionIssuer ??= createFrameworkHubDeviceAssertionIssuer({
    repoRoot: REPO_ROOT,
    env: process.env,
  });
  return hubDeviceAssertionIssuer(input);
}
...
const DEFAULT_CASE_DELAY_MS = optionalIntEnv('PSFN_CASE_DELAY_MS', 10000);
const DEFAULT_STEP_DELAY_MS = optionalIntEnv('PSFN_STEP_DELAY_MS', DEFAULT_CASE_DELAY_MS);
const MODEL_LANE_DISPATCH_TIMEOUT_MS = optionalIntEnv(
  'PSFN_MODEL_LANE_DISPATCH_TIMEOUT_MS',
  300000,
);
...
function listBeadsIssuesByTitle(title, limit = Number.POSITIVE_INFINITY) {
  if (!BEADS_DIR) {
    throw new Error('PSFN_BEADS_DIR is required for Beads shakedown cases');
  }
  const raw = execFileSync('bd', [
    'list',
    '--all',
    '--title',
    title,
  ...
  if (!Array.isArray(issues)) {
    throw new Error('bd list --all returned a non-array JSON payload');
  }
  return issues
...
function closeShakedownIssueResidues(issues, reason) {
  ...
  for (const issue of rowsOf(issues)) {
    ...
    if (!isBeadsIssueId(id) || status === 'closed') continue;
    try {
...
function cleanupCapabilityMatrixIssue(issueId, expectedTitle) {
  ...
  if (issue.status !== 'closed') {
    execFileSync(
      ...
      { cwd: REPO_ROOT, encoding: 'utf8', env: { ...process.env, BEADS_DIR } },
    );
  }
...
function cleanupCapabilityMatrixIssuesByTitle(expectedTitle) {
  const matches = listBeadsIssuesByTitle(expectedTitle);
...
function deriveIssueReadSyncProof(archiveToolMessages) {
  ...
  const showPayload = payloads.find((payload) => {
    ...
    if (typeof payload.target === 'string' && payload.target.trim().length > 0) {
      return readyId ? payload.target === readyId : isBeadsIssueId(payload.target);
    }
  ...
  const showId = isBeadsIssueId(showPayload?.target)
    ? showPayload.target
...
async function chatCase(input) {
  ...
  const dispatch = prepareCaseChatDispatch({
    ...
    resolveDispatchAuth: input.resolveDispatchAuth,
    sessionId: input.sessionId,
    privacy: input.privacy ?? 'private',
    extraHeaders: withTestingHarnessProvenance(input.headers, {
  ...
  while (submitAttempts < maxSubmitAttempts) {
    ...
    matchingTurn = await waitForMatchingTurnRecord(
      ...
      undefined,
      input.signal,
      typeof input.resolveAttemptHeaders === 'function',
    );
  ...
  return {
    sessionId: input.sessionId,
    ...(typeof input.resolveDispatchAuth === 'function' ? { apiUserId } : {}),
    busyRetries,
    submitAttempts,
    busyRejected,
    busyObservedAtMs,
    ...
    response,
    turnRecord: matchingTurn ?? (isAgentBusyResponse(response) ? null : lastTurnForSession(input.sessionId, apiUserId)),
  };
}
...
function buildApprenticeCases(ctx) {
  ...
  const editSourceUrl = 'https://storage.googleapis.com/falserverless/example_inputs/nano-banana-edit-input.png';
  ...
  return [
    ...
    },
    {
      ...
      validateParsedAssistant: ({ parsedAssistant }) => {
        const failures = [];
        if (!isBeadsIssueId(parsedAssistant?.issueId)) {
          failures.push('issue_create_update issueId must be a canonical Beads issue id');
        }
      ...
      after: async () => {
        const beforeCleanup = listBeadsIssuesByTitle(issueTitle, 3);
        const cleanup = closeShakedownIssueResidues(beforeCleanup, 'Shakedown issue_create_update cleanup');
        const afterCleanup = listBeadsIssuesByTitle(issueTitle, 3);
        return {
...
function buildCoverageCases(ctx) {
  ...
  const orientMarker = `Preserve verified orient persistence from shakedown run ${ctx.runToken}.`;
  ...
  return [
    ...
    },
    {
      ...
      message:
        ...
        + 'If the tool reports no durable change, that typed no-op is the expected outcome. '
        + 'Return only a JSON object with keys appended and note.',
      validateParsedAssistant: ({ parsedAssistant }) => {
        const failures = [];
        if (typeof parsedAssistant?.appended !== 'boolean') {
          failures.push('orient_append appended must be a boolean');
        }
        if (parsedAssistant?.appended === false) {
          const note = typeof parsedAssistant?.note === 'string' ? parsedAssistant.note : '';
          if (!/no durable change/i.test(note)) {
            failures.push('orient_append no-op must report the typed no durable change reason');
          }
        }
        return failures;
      },
      ...
      validateSideEffects: ({ sideChecks, parsedAssistant }) => {
        if (sideChecksContainText(sideChecks, orientMarker)) {
          return [];
        }
        const note = typeof parsedAssistant?.note === 'string' ? parsedAssistant.note : '';
        if (/no durable change/i.test(note)) {
        ...
        return ['orient_append must persist the appended orient marker or return a typed no durable change'];
      },
    },
    {
    ...
    },
    {
      ...
      actionSuccessKeys: [],
      message:
        'Call memory exactly once with action "redact", memory_id "00000000-0000-4000-8000-000000000000", '
        + 'operation "delete", and reason "matrix retired alias guard". '
        + 'This action is expected to be rejected as retired. '
        + 'Return only a JSON object with keys redacted and note.',
      timeoutMs: 60000,
      validateParsedAssistant: ({ parsedAssistant, archiveToolMessages }) => {
        const toolText = archiveToolMessages
          .filter((entry) => entry?.toolName === 'memory')
          .map((entry) => entry?.contentText ?? '')
          .join('\n');
        const note = typeof parsedAssistant?.note === 'string' ? parsedAssistant.note : '';
        const failures = [];
        if (parsedAssistant?.redacted !== false) {
          failures.push('memory_redact redacted must be false for the retired alias');
        }
        if (!/action=redact is retired/u.test(`${toolText}\n${note}`)) {
          failures.push('memory_redact must report the exact retired-action blocker');
        }
        return failures;
      },
    },
  ...
  return [
    {
      ...
      validateParsedAssistant: ({ parsedAssistant, archiveToolMessages }) => {
        ...
        const failures = [];
        if (!isBeadsIssueId(issueId)) {
          failures.push('issue_close_cycle issueId must be a canonical Beads issue id');
        }
      ...
      after: async () => ({
        issues: listBeadsIssuesByTitle(issueTitle, 3),
      }),
      validateSideEffects: ({ sideChecks, parsedAssistant }) => {
        const issueId = typeof parsedAssistant?.issueId === 'string'
          ? parsedAssistant.issueId.trim()
...
function buildCases(ctx) {
  ...
  const memoryTierCases = buildMemoryTierCases(ctx, {
    adminBase: ADMIN_BASE,
    approveOperatorConfirmation: async (confirmationId, signal) => {
  ...
  const coverage = [
    ...
    ...buildHardeningCases(ctx, {
      ...
      modelLaneDispatchTimeoutMs: MODEL_LANE_DISPATCH_TIMEOUT_MS,
      modelLaneStepDelayMs: DEFAULT_STEP_DELAY_MS,
    }),
  ];
  ...
  if (typeof testCase.execute === 'function') {
    ...
    for (let index = 0; index < caseMessages.length; index += 1) {
      ...
      if (index + 1 < caseMessages.length) {
        await sleep(step.postStepDelayMs ?? testCase.stepDelayMs ?? DEFAULT_STEP_DELAY_MS, signal);
      }
    }
  }
  if (!outcome) {
...
async function main() {
  ...
  const operatorHealth = await fetchJson(ADMIN_READINESS_URL);
  ...
  const contextSummary = {
    ...
    harnessManifestId: HARNESS_MANIFEST_ID,
  };
  const outputBase = {
    startedAt,
    target: TARGET,
    ...
    apiBase: API_BASE,
    adminBase: ADMIN_BASE,
    adminReadinessUrl: ADMIN_READINESS_URL,
    bootstrap,
    apiHealth,
    operatorHealth,
    adaptiveTools,
  ...
  const catalogToolNames = requireAdaptiveToolCatalog(adaptiveTools);
