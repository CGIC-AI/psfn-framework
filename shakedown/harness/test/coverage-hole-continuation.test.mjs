...
const root = mkdtempSync(join(tmpdir(), 'coverage-hole-continuation-'));
...
try {
  ...
  const result = spawnSync('bash', [matrix], {
    ...
    env: {
      ...
      STUB_HARNESS_CASE_STATUS: 'coverage_hole',
      STUB_HARNESS_FAILURE_REASON: failureReason,
    },
  });
  ...
  for (const [label, tier] of [
    ...
    assert(artifact.results[0]?.caseStatus === 'coverage_hole', `${label} lost hole status`);
    assert(artifact.results[0]?.failureReason === failureReason, `${label} lost named reason`);
  }
  assert(readFileSync(stateFile, 'utf8').trim() === originalTier, 'original tier was not restored');
  console.log('coverage-hole continuation test passed');
} finally {
