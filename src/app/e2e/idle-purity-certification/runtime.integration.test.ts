...
describe('idle-purity real quiet-runtime certification', () => {
  ...
  let processHarness: IcpSingleCompanionFeatureOffHarness | null = null;
  ...
  it('boots enabled automata with no work due and observes no durable churn', async () => {
    ...
    processHarness = await startIcpSingleCompanionFeatureOffHarness({ fixture });
    await expect(processHarness.agent.pendingBackgroundWorkCount()).resolves.toBe(0);
