...
const sessionHmacDescriptors = [
  defineAuditedMethod({
    ...
    decode: gatewayMethodParamDecoders['session.hmac.sign'],
  ...
  defineAuditedMethod({
    ...
    decode: gatewayMethodParamDecoders['session.hmac.verify'],
...
export function registerSessionHmacMethods(runtime: GatewayMethodRuntime): void {
  for (const descriptor of sessionHmacDescriptors) {
    runtime.target.addMethod(
      descriptor.name,
      (params: unknown) => descriptor.handler(descriptor.decode(params), runtime),
    );
  }
}
