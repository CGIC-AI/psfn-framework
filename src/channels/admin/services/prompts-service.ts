...
export class AdminPromptsDataService implements AdminPromptsService {
  ...
  createPromptLayer(body: string): PromptUpdateResult {
    ...
    const name = (params.get('name') ?? '').trim();
    const content = params.get('content') ?? '';
    const channelType = params.get('channelType') ?? undefined;
    const taskKind = params.get('taskKind') ?? undefined;
    const priorityRaw = params.get('priority');
    const priority = priorityRaw != null ? parseInt(priorityRaw, 10) : 0;

    const VALID_TYPES = ['runtime', 'channel', 'task'];
    ...
    try {
      const layer = promptStore.create({
        type: type as 'runtime' | 'channel' | 'task',
        name,
        content,
        identifier: resolvedMetadata.metadata.identifier,
        role: resolvedMetadata.metadata.role,
