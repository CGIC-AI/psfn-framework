...
function executeContactSetTrust(
...
function executeContactSetChannelPrivacy(
  contactStore: ContactStore,
  params: {
    contactId?: string;
    channel?: string;
...
function lookupContact(contactStore: ContactStore, id: string) {
...
function executeContactLookup(
  ...
  const contact = lookupContact(contactStore, id);
  if (!contact) {
...
function executeContactLinkIdentity(
  contactStore: ContactStore,
  params: {
    contactId?: string;
    channel?: string;
...
function executeContactList(contactStore: ContactStore): AgentToolResult<{ isError?: boolean }> {
...
async function executeUnifiedContactAction(
  ...
  switch (action) {
    case 'list':
      return executeContactList(contactStore);
    case 'lookup':
      return executeContactLookup(contactStore, params);
    case 'note':
      return executeContactNote(contactStore, params);
    case 'set_trust':
      return executeContactSetTrust(contactStore, params as ContactSetTrustParams);
    case 'link_identity':
      return executeContactLinkIdentity(contactStore, params);
    case 'set_channel_privacy':
      return executeContactSetChannelPrivacy(contactStore, params);
  }
}
...
export function createContactSetTrustTool(contactStore: ContactStore): AgentTool<any> {
  return {
    ...
    execute: async (
      ...
      try {
        return executeContactSetTrust(contactStore, params);
      } catch (error) {
