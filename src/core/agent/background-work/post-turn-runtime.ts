...
import type { IntentionPostTurnHookContext } from '../substrate-agent/post-turn-actions.js';
...
export interface PostTurnBackgroundRuntimeDependencies {
  ...
  runIntentionPostTurnHooks: (context: IntentionPostTurnHookContext) => Promise<void>;
...
export async function executePostTurnBackgroundWork(
  ...
  switch (payload.kind) {
    ...
    case 'intention_post_turn_hooks':
      await dependencies.runIntentionPostTurnHooks(rehydrateIntentionContext(record, payload));
      return;
    ...
    case 'auto_compaction':
      await dependencies.sessionManager.scheduleAutoCompactionBetweenTurns({
        ...
        ...(payload.compactionPromptText
          ? { compactionPromptText: payload.compactionPromptText }
          : {}),
        ...(payload.icpCorrelation ? { icpCorrelation: payload.icpCorrelation } : {}),
      });
      return;
