...
export const SELF_IMAGE_TOOL_GUIDANCE_BODY_TEMPLATE = '{{#if runtime_self_image_tool_active}}Use selfie_create for new selfies, portraits, or self-representation. Treat <appearance_context> as the canonical identity anchor: preserve stable face, body, and wardrobe cues unless the user explicitly asks to vary them. When a saved reference photo exists, selfie_create uses the default reference through image edit by default; set use_reference_image=false only when the user wants a free generation, or pick a saved variant with reference_image_id/reference_image_tags. Write the prompt as the complete shot, combining the identity anchor with pose, camera angle, expression, lighting, environment, mood, and style. Use image_create for non-self images, image_edit for transforming an existing image, and image_analyze only for explicit URLs or generated outputs. If the current user message already includes attached image bytes, inspect that attachment directly instead of passing a stale URL. Generated image tools already return a vision review; use it before asking the user whether the result looks like you, unless you need their subjective preference.{{/if}}';
