...
export const INTERNAL_STATE_BODY_TEMPLATE = '{{#if runtime_internal_state_present}}Current affect: {{runtime_internal_state_emotional_prefix}}{{runtime_internal_state_emotional_mood_valence_label}} and {{runtime_internal_state_emotional_mood_arousal_label}}{{runtime_internal_state_emotional_secondary_clause}}.{{runtime_internal_state_emotional_validation_clause}}\nThinking state: {{runtime_internal_state_cognitive_processing_quality}}, {{runtime_internal_state_cognitive_certainty_label}} certainty, {{runtime_internal_state_cognitive_topic_engagement_label}} engagement.\nAttention: {{runtime_internal_state_attention_conversation_trajectory}}, {{runtime_internal_state_attention_active_concern_count}} open thread{{runtime_internal_state_attention_active_concern_plural_suffix}}, {{runtime_internal_state_attention_pending_follow_up_count}} pending follow-up{{runtime_internal_state_attention_pending_follow_up_plural_suffix}}.\nRelationship baseline: {{runtime_internal_state_relational_trust_level}} trust, {{runtime_internal_state_relational_recent_interaction_frequency_label}} contact, {{runtime_internal_state_relational_last_seen_label}}.{{/if}}';

export const SELF_IMAGE_TOOL_GUIDANCE_BODY_TEMPLATE = '{{#if runtime_self_image_tool_active}}Use selfie_create for new selfies, portraits, or self-representation. Treat <appearance_context> as the canonical identity anchor: preserve stable face, body, and wardrobe cues unless the user explicitly asks to vary them. When a saved reference photo exists, selfie_create uses the default reference through image edit by default; set use_reference_image=false only when the user wants a free generation, or pick a saved variant with reference_image_id/reference_image_tags. Write the prompt as the complete shot, combining the identity anchor with pose, camera angle, expression, lighting, environment, mood, and style. Use media action="generate" for non-self images, media action="edit" for transforming an existing image, and media action="analyze" only for explicit URLs or generated outputs. If the current user message already includes attached image bytes, inspect that attachment directly instead of passing a stale URL. Generated image tools already return a vision review; use it before asking the user whether the result looks like you, unless you need their subjective preference.{{/if}}';
