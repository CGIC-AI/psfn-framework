  ...
  it('preserves tool arguments, results, and rationale in the turn record', () => {
    const record = buildTurnRecord({
      ...
      turnMessages: [
        ...
        {
          role: 'toolResult',
          toolCallId: 'call-dependent',
          toolName: 'fs',
          outcome: 'dependency_skip',
          isError: true,
          timestamp: 1_700_000_000_170,
          content: [{
            type: 'text',
    ...
    expect(record.toolCalls).toEqual([
      ...
      },
      {
        ...
        outcome: 'dependency_skip',
        isError: true,
        provenanceRefs: ['source:tool:fs|invocation:call-dependent'],
        resultText: 'Skipped because an earlier sequential tool call failed. Read the tool result and retry only the needed follow-up call.',
        details: {},
      },
    ]);
  });
