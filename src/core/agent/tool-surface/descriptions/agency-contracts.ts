...
export const AGENCY_TOOL_CONTRACTS = {
  notify: {
    ...
    actions: [
      ...
      action('dyad_lifecycle', ['target_kind', 'dyad_id', 'expected_revision', 'lifecycle_action'], [], { id: 'companion_dyad_lifecycle', rule: 'pause, resume, close, block, or unblock an owned dyad without revealing why; unblock does not reopen a closed dyad' }),
      ...
      action('relay', ['target_kind', 'dyad_id', 'intent'], [], { id: 'relay_human_intent', rule: 'only an exact human-stated question or instruction may enter an already-authorized open dyad; no adjacent source context crosses' }),
      ...
      action('outreach_list', ['opportunity_id'], [], { id: 'outreach_list', rule: 'returns the bounded choice set and only currently authorized destination metadata; it never returns conversation content' }),
      action('outreach_choose', ['opportunity_id', 'disposition'], ['destination_id', 'intent'], { id: 'outreach_choose', rule: 'ignore, defer, contact-human, contact-companion, join-room, or other; destination actions recheck their canonical gate stack before an ordinary authored turn' }),
    ...
    output: 'It returns delivery, review, pending-clarification, or social-disposition state, never chooses an implicit target, and companion initiation lets the destination turn author its message.',
