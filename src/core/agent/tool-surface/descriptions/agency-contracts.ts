...
export const AGENCY_TOOL_CONTRACTS = {
  notify: {
    ...
    actions: [
      ...
      action('outreach_send', ['message'], [], { id: 'outreach_send', rule: 'only inside an outreach moment about one person: message is exactly what you want to say to them; the runtime delivers it to your one-to-one conversation with them or through companion messaging, never to a group' }),
      action('outreach_later', [], [], { id: 'outreach_later', rule: 'only inside an outreach moment: not now, ask me again later; to decline, simply do not call notify' }),
    ...
    output: 'It returns delivery, review, pending-clarification, or outreach-answer state, never chooses an implicit target, and companion initiation lets the destination turn author its message.',
