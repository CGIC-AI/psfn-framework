...
export function rowToContact(row: ContactRow, identities: ContactIdentityRow[], conversationChannels: ContactChannelActivityRow[]): Contact {
  ...
  const contact: Contact = {
    ...
    ...(normalizeTrimmed(row.gender ?? undefined) ? { gender: normalizeTrimmed(row.gender ?? undefined) } : {}),
    ...(normalizeTrimmed(row.pronouns ?? undefined) ? { pronouns: normalizeTrimmed(row.pronouns ?? undefined) } : {}),
