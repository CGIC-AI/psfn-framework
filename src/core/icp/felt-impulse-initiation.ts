  ...
  return {
    onLeverSignal: async (signal): Promise<IcpFeltImpulseOutcome> => {
      ...
      if (peers.length === 0) {
        ...
        await emitOutcome({ outcome: 'no_eligible_peer', reason: 'missing_companion_channel_contacts' });
      ...
      const result = await deps.sourceRuntime.submit({
        ...
        sourceRecordId: `felt-impulse:${signal.lever}:${signal.firedAtMs}`,
