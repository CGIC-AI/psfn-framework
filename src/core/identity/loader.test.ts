...
describe('loadOrInitializeCharacterCard', () => {
  ...
  it('fails closed when the companion file is missing', () => {
    const root = makeTempDir();
    const path = join(root, 'nested', 'companion.json');

    expect(() => loadOrInitializeCharacterCard(path)).toThrow(
      `Missing character card at ${path}`,
    );
    expect(existsSync(path)).toBe(false);
  });

  it('does not silently adopt a legacy character.json file into companion.json', () => {
    const root = makeTempDir();
    ...
    expect(() => loadOrInitializeCharacterCard(companionPath)).toThrow(
      `Missing character card at ${companionPath}`,
    );
