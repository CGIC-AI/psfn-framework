...
const SYSTEM_LANGUAGE_SCHEMA_VERSION = 1;
const MAX_TEMPLATE_CHARS = 800;
...
type SystemLanguageTemplateMap = Record<SystemLanguageTemplateKey, string>;
...
export const DEFAULT_SYSTEM_LANGUAGE_TEMPLATES: Readonly<SystemLanguageTemplateMap> = Object.freeze({
  ...
  'substrate_health.header': '[Substrate health]',
  'substrate_health.overall.healthy': 'Available probes report no substrate degradation.',
  'substrate_health.overall.degraded': 'One or more substrate signals need attention.',
  'substrate_health.overall.unavailable': '{{unavailable_reason}}; do not assume the substrate is nominal.',
  'substrate_health.subsystem_missing_probe': 'no probe snapshot is available.',
  'substrate_health.subsystem_degraded_suffix': '; use simpler paths and avoid optional costly work until it recovers.',
  'substrate_health.gateway_degraded_suffix': '; replies or tool calls that depend on the gateway may be slow or fail.',
...
const ALLOWED_PLACEHOLDERS: Record<SystemLanguageTemplateKey, readonly string[]> = {
  ...
  'substrate_health.overall.healthy': [],
  'substrate_health.overall.degraded': [],
  'substrate_health.overall.unavailable': ['unavailable_reason'],
  'substrate_health.subsystem_missing_probe': [],
  'substrate_health.subsystem_degraded_suffix': [],
  'substrate_health.gateway_degraded_suffix': [],
  'wake_return.header': [],
...
function cloneDefaultTemplates(): SystemLanguageTemplateMap {
...
function validateTemplateValue(
  ...
  if (value.length > MAX_TEMPLATE_CHARS) {
    diagnostics.push({
      code: 'invalid_template',
      key,
      message: `system language template "${key}" exceeds ${MAX_TEMPLATE_CHARS} characters`,
    });
  }
...
function renderTemplate(
  ...
  return {
    text: normalizeTemplateText(text),
    diagnostics,
  };
}
...
export function renderSystemLanguageTemplateWithDiagnostics(
  ...
  const rendered = renderTemplate(key, sourceTemplate, variables);
  ...
  const fallback = renderTemplate(key, DEFAULT_SYSTEM_LANGUAGE_TEMPLATES[key], variables);
  return {
    text: fallback.text,
    diagnostics: [
