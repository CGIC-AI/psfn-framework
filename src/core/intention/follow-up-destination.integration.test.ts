  ...
  it.each([
    ...
    const complete = vi.fn().mockResolvedValue({
      content: JSON.stringify({ decisions: [{
        ...
        type: 'concern', priority: 'medium', timing: 'soon', reason: 'Their week has been hard.',
        concern: { title: 'Check how their hard week turns out' },
      }] }),
    ...
    wirePostTurnRuntime({
      ...
      runtimeOptions: fromAny({
        ...
        onIntentionConcernDecision: hooks.onIntentionConcernDecision,
    ...
    try {
      ...
      // An unauthorized follow-up target rejects only that follow-up (vcq8v):
      // the concern decided in the same appraisal is still recorded.
      expect((await ports.concernStore.list()).map(concern => concern.text))
        .toEqual([expect.stringContaining('Check how their hard week turns out')]);
