...
export function wirePostTurnRuntime(
  ...
  // Outbound provenance + activation gates, extracted to
  // post-turn-outbound-gates.ts (charter 12.1 split, emh3p.4).
  const {
  ...
  } = createPostTurnOutboundGates({
    runtimeOptions,
    telemetryEventBus,
    lastIntentionFollowUpActivationByChannel,
  });
