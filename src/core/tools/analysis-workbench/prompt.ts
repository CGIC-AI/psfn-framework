...
function buildBasePrompt(
  ...
  return [
    ...
    '',
    '### Session (read-only)',
    '- `await session_messages(channelId, limit?)` — Get recent messages from a channel this conversation may read (others are refused), returns array of {role, content, timestamp}',
    '- `await session_search(query, limit?, options?)` — Keyword search historical transcripts; options accepts only {channelId} as a scope filter; returns {summary, totalHits, gatedOutCount, hits}',
    '',
