...
function parseEmbeddingHealthCount(value: number | string, field: string): number {
  const parsed = typeof value === 'number' ? value : Number(value);
  if (!Number.isSafeInteger(parsed) || parsed < 0) {
...
export class PostgresEpisodicStore implements
  ...
  async listEpisodeEmbeddingTargets(
    ...
    const profile = this.normalizeEmbeddingProfile(input.profile);
  ...
  async getEpisodeEmbeddingIndexHealth(
    ...
    const profile = this.normalizeEmbeddingProfile(inputProfile);
    ...
    return {
      total: parseEmbeddingHealthCount(row.total_count, 'total'),
      current: parseEmbeddingHealthCount(row.current_count, 'current'),
      missing: parseEmbeddingHealthCount(row.missing_count, 'missing'),
      stale: parseEmbeddingHealthCount(row.stale_count, 'stale'),
      failed: parseEmbeddingHealthCount(row.failed_count, 'failed'),
    };
  }
  ...
  private normalizeEmbeddingProfile(profile: EpisodeEmbeddingTargetListInput['profile']): EpisodeEmbeddingTargetListInput['profile'] {
