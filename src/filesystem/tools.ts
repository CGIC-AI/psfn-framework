...
export function createFsTool(ops: FilesystemOperations): AgentTool<any> {
  return {
    ...
    parameters: Type.Object({
      ...
      path: Type.Optional(Type.String({ description: 'Workspace-relative path for action=read, action=write, or action=edit.' })),
      glob: Type.Optional(Type.String({ description: 'Workspace-relative glob for action=list or action=search. Defaults to **/*.' })),
      ...
      query: Type.Optional(Type.String({ description: 'Used with action=search. Literal text or regex pattern to find.' })),
      ...
      content: Type.Optional(Type.String({ description: 'Used with action=write. Full replacement file content.' })),
      overwrite: Type.Optional(Type.Boolean({
        description: 'Used with action=write. Required to replace an existing file with different content.',
      })),
      old_text: Type.Optional(Type.String({ description: 'Used with action=edit. Exact existing text to replace.' })),
      new_text: Type.Optional(Type.String({ description: 'Used with action=edit. Replacement text.' })),
