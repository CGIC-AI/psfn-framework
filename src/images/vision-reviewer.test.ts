...
describe('DefaultImageVisionReviewer', () => {
  ...
  it('falls back to direct fetch for configured ComfyUI hosts when gateway fetch fails', async () => {
    ...
    const reviewer = new DefaultImageVisionReviewer(
      {
        primaryProvider: 'openrouter',
        comfyUiBaseUrl: 'https://comfy.local.vega.nyc',
      } as any,
      {
        binaryFetcher,
        fetchImpl,
    ...
    const result = await reviewer.analyze({
      imageUrls: ['https://comfy.local.vega.nyc/view?filename=review.png'],
      prompt: 'a new selfie',
      mode: 'create',
    });
    ...
    expect(fetchImpl).toHaveBeenCalledWith(
      'https://comfy.local.vega.nyc/view?filename=review.png',
