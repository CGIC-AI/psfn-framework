...
describe('extractMentionOnlyContactCandidate', () => {
  it('extracts a family member name from relationship mentions', () => {
    const candidate = extractMentionOnlyContactCandidate({
      fact: makeFact("Vega's sister Alex is moving to Seattle", { tags: ['family'] }),
      canonicalContactName: 'Vega',
  ...
  it('extracts names that appear before the relationship phrase', () => {
    const candidate = extractMentionOnlyContactCandidate({
      fact: makeFact("Alex is Vega's friend from college"),
      canonicalContactName: 'Vega',
      companionName: 'Purrsephone',
    });
  ...
  it('excludes canonical user and companion names from fallback matching', () => {
    expect(extractMentionOnlyContactCandidate({
      fact: makeFact('Purrsephone is Vega\'s companion'),
      canonicalContactName: 'Vega',
      companionName: 'Purrsephone',
    })).toBeUndefined();

    expect(extractMentionOnlyContactCandidate({
      fact: makeFact('Vega helped their family with dinner', { tags: ['family'] }),
      canonicalContactName: 'Vega',
      companionName: 'Purrsephone',
    })).toBeUndefined();
  });
...
describe('resolveMentionOnlyContactForFact', () => {
  ...
  it('promotes an existing contact relationship from stronger recurring evidence', () => {
    ...
    const resolved = resolveMentionOnlyContactForFact({
      fact: makeFact("Vega's sister Alex is visiting this weekend", { tags: ['family'] }),
...
describe('MemoryExtractor mention-only contacts', () => {
  ...
  it('creates and relinks a mention-only contact only after recurring evidence', async () => {
    ...
    await (extractor as any).processFact(
      makeFact("Vega's sister Alex is moving to Seattle", { tags: ['family'] }),
    ...
    expect(memoryStore.getMemoriesByContact(primary.id, 10).map(memory => memory.text)).toContain(
      "Vega's sister Alex is moving to Seattle",
    );
    ...
    expect(alexMemories.map(memory => memory.text)).toContain("Vega's sister Alex is moving to Seattle");
  ...
  it('does not create a contact from a one-off relational mention', async () => {
    ...
    await (extractor as any).processFact(
      makeFact("Vega's coworker Jordan likes this cafe", { tags: ['coworker'] }),
