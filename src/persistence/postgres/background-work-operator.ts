 ...
 * never the payload. Retirement is terminal (`stale_discarded`) and keeps the
 * row's last reason code (why it got stuck); the CLI's audit file records that
 * an operator retired it. It never touches a job whose lease is still live, so
 * it cannot race the process that holds it.
 */
