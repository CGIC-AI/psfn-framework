 ...
 * reconciliation and lifecycle-driven session fencing, but the runtime role
 * must not receive UPDATE on the authority_state table merely to satisfy
 * PostgreSQL's row-lock privilege requirement. This zero-argument SECURITY
 * DEFINER function exposes only the two values needed by the broker while
 * retaining the exclusive row lock until the caller's transaction ends.
 */
