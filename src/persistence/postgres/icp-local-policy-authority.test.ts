...
function fakePool(query = policyQuery()) {
...
describe('PostgresIcpLocalPolicyAuthority', () => {
  ...
  it('retains exact local locks until bound release and rejects replay', async () => {
    ...
    await expect(authority.acquire(request)).resolves.toEqual({
      acquired: true,
      holdId: HOLD_ID,
      expiresAtMs: 3_000,
    });
    ...
    await expect(authority.release({ holdId: HOLD_ID, payloadDigest: DIGEST, nonce: NONCE }))
  ...
  it('rolls back every retained client on expiry, disconnect cleanup, and shutdown', async () => {
    ...
    try {
      ...
      await authority.acquire(request);
      nowMs = 3_000;
      await vi.advanceTimersByTimeAsync(1_000);
      expect(fixture.query).toHaveBeenCalledWith('ROLLBACK');

      await authority.acquire({ ...request, nonce: '99999999-9999-4999-8999-999999999999', nowMs: 4_000, expiresAtMs: 5_000 });
      await authority.releaseAll();
      expect(fixture.query.mock.calls.filter(([sql]) => sql === 'ROLLBACK')).toHaveLength(2);

      await authority.acquire({ ...request, nonce: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', nowMs: 6_000, expiresAtMs: 7_000 });
