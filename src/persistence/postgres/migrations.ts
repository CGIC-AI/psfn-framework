...
export const POSTGRES_BUZZ_RECOVERY_MIGRATIONS: readonly string[] = [
  ...
  `
  CREATE TABLE IF NOT EXISTS buzz_causal_roots (
    community TEXT NOT NULL,
    companion_id TEXT NOT NULL,
    root_event_id TEXT NOT NULL,
    human_author_pubkey TEXT NOT NULL,
    observed_at_ms BIGINT NOT NULL,
    PRIMARY KEY (community, companion_id, root_event_id),
    CHECK (root_event_id ~ '^[a-f0-9]{64}$'),
    CHECK (human_author_pubkey ~ '^[a-f0-9]{64}$')
  );
  `,
  `
  CREATE TABLE IF NOT EXISTS buzz_causal_edges (
    community TEXT NOT NULL,
    companion_id TEXT NOT NULL,
    ...
    event_id TEXT NOT NULL,
    observed_at_ms BIGINT NOT NULL,
    PRIMARY KEY (community, companion_id, chain_id, parent_event_id, author_pubkey),
    UNIQUE (community, companion_id, event_id),
    CHECK (chain_id ~ '^[a-f0-9]{64}$'),
    CHECK (parent_event_id ~ '^[a-f0-9]{64}$'),
