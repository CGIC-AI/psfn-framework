...
describe('PostgresModelUsageStore reconciliation', () => {
  ...
  it('prices a registry-matched historical success before budget preflight', async () => {
    if (!harness) throw new Error('Postgres test harness is unavailable');
    const { databaseUrl } = await harness.createDatabase();
    const pool = createPostgresPool(databaseUrl, {
  ...
  it('projects UTC daily and monthly budgets from canonical chat and completion attempts', async () => {
    ...
    try {
      ...
      await store.recordUsageEvent({
        logicalCallId: 'today-local-embedding-free',
        attempt: 1,
        recordedAtMs: Date.parse('2026-03-06T09:30:00.000Z'),
        startedAtMs: Date.parse('2026-03-06T09:29:59.990Z'),
        completedAtMs: Date.parse('2026-03-06T09:30:00.000Z'),
        status: 'success',
        settlement: 'unknown',
        callKind: 'embedding',
