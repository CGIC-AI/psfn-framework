...
describe('operator alerts', () => {
  it('formats stable prompt-generation failure alerts for ntfy delivery', () => {
    const text = formatPromptGenerationFailureAlert(makeEvent(), 'Purrsephone');

    expect(text).toContain('Purrsephone prompt generation failed after exhausting configured fallback.');
  ...
  it('sends a priority-5 ntfy alert for terminal prompt-generation failures', async () => {
    ...
    const handler = createPromptGenerationFailureAlertHandler(notifier, 'Purrsephone');

    await handler(makeEvent());

    expect(notifier.notify).toHaveBeenCalledWith(expect.objectContaining({
      title: 'Purrsephone prompt generation failure',
      priority: 5,
