...
describe('skills loader', () => {
  ...
  it('resolves precedence directories with defaults first and extras appended', () => {
    ...
    expect(directories[0]?.relativePath).toBe('psfn/skills');
  ...
  it('keeps higher-precedence skill definitions when names collide', () => {
    ...
    try {
      writeSkill(root, 'psfn/skills/conversation', `
---
name: conversation
description: psfn override
---
# PSFN
`);
...
`);
      ...
      expect(deduped.entries[0]?.description).toBe('psfn override');
      expect(deduped.entries[0]?.relativePath).toContain('psfn/skills/conversation/SKILL.md');
