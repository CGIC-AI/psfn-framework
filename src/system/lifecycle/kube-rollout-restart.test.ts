...
describe('combineKubeSelfManagementExecutors', () => {
  it('delegates each action to the first executor that supports it', async () => {
    const diagnose = {
  ...
  it('throws for an unsupported action', async () => {
    ...
    await expect(combined.execute(RESTART_REQUEST)).rejects.toThrow(/not configured in this runtime/);
  });
});
