...
/**
 * Compose a set of executors into one that delegates by action. The first
 * executor claiming support for an action handles it. Used so the controller's
 * single executor can serve both read-only diagnostics and the mutating
 * rollout restart.
 */
export function combineKubeSelfManagementExecutors(
  ...
  return {
    supports: (action: KubeSelfManagementAction): boolean =>
      executors.some(executor => executor.supports(action)),
    execute: async (
      request: KubeSelfManagementRequest,
    ): Promise<KubeSelfManagementExecutionResult> => {
      const executor = executors.find(candidate => candidate.supports(request.action));
      if (!executor) {
