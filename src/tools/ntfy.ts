...
class DefaultNotifyDispatcher implements NotifyDispatcher {
  private readonly briefNotifier: NtfyNotifier;
...
export function createOutboundDockNotifySender(dock: ChannelOutboundDock): NotifyChannelSender {
  return {
    send: async ({ channel, target, message }) => {
      if (channel !== 'discord') {
        throw new Error(`${channel} delivery is not wired`);
      }
      await dock.outbound.sendText({ channelId: target }, message);
    },
  };
}
