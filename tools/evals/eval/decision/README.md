...
| --- | --- |
| `npm --prefix tools/evals run eval:decision:shadow-report -- --input <decision-shadow.jsonl> [--out report.json]` | Per-site agreement, confusion, latency p50/p95, Jev cost and failures from runtime shadow records (`<companionDataDir>/state/decision-shadow.jsonl`). |
| `npm --prefix tools/evals run eval:decision:bakeoff -- [--live --local-endpoint <url> --local-model <id>] [--jev-model typesafe/jev-1.13] [--jev-snapshot <dated id>] [--run-id <id>] [--output-dir <dir>]` | Runs the labeled cases in `bakeoff-cases.ts` against a local OpenAI-compatible model and Jev; reports accuracy, Brier and reliability, latency and cost per site. Without `--live` it uses deterministic fixture backends. |
| `npm --prefix tools/evals run eval:decision:qao-jev-judge -- --live --council <qao judge council run.json> [--out <path>]` | Scores each QAO rubric axis with Jev and compares it to the council mean (mean absolute difference, exact-level and pass/fail agreement). |
