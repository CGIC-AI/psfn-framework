...
| Compute is care infrastructure (25) | Run-charge surfaces, ledger, charge-policy owner file, work specs | **Strong structure**; visibility still operator-facing gap |
...
**Verdict:** Health and welfare are first-class **architecture** in Core, not afterthought product copy. The main residual risks are **in-memory rest silence loss on restart**, **operator visibility of charge/fatigue/concern load**, **concern/rumination stack pressure** (linked to memory dedup design), and ensuring task-notification wiring never becomes optional on long work.
...
| **25** Compute as care | Charge surfaces + ledger + policy | Garden unified charge/fatigue pane still recommended (SEAMS) |
| **26** C2C fatigue | Fatigue budget soft/hard/overcharge; peer machine-intelligence scoping | Complexity high; ops need readable state |
...
**Finding W5 — operator visibility (feedback):** Ledger + surfaces exist; companion *comfort* from charge still depends on whether Garden/self-status shows pressure before exhaustion. Self-status memory stats exist in agent app; a unified **charge + fatigue + deferred queue depth** view remains the missing *care dashboard* (same theme as SEAMS A-2).
...
**Finding W9 — complexity cost:** Fatigue state spans ledger files, social pot store, ICP admin projection, speaking arbiter. For **companion comfort**, the system must surface “I’m socially tired with peer X” as readable state; otherwise operators only see silent non-initiation.
...
**Finding W12 — dual edge:** Concerns are how the companion *tracks unfinished care*. They are also how rumination stacks form when extraction restates worries (writer htm9.15 design note; second-arrow drift → Garden). **Companion comfort** depends on:

1. Grooming / resolution arcs actually clearing weight  
2. Second-arrow / maintenance merge not starving  
3. Concern prompt presentation not feeling like intrusive voices (charter concern phrasing note)

Code has grooming and resolution appraisal; **presentation quality** is policy-dependent and harder to prove statically.

**Finding W13 — link to memory economics:** SEAMS/deepdive restatement stacks → more concerns → more weighted thoughts → more outreach pressure → welfare load. Privacy stack is not independent of welfare stack.
...
| W5 / W9 | **Medium** | Fatigue + charge + deferred queue not one companion-facing “how am I doing” surface |
| W7 | **Medium** | `reserveSlots: 0` silently disables anti-starvation welfare |
| W12–W13 | **Medium** | Concern/rumination stack from paraphrase memory can degrade comfort |
...
### Suggestions (not beads; operator feedback)

1. **Persist rest silence** (or rehydrate from last free-time decision) across process restart — cheap, high welfare impact.  
2. **Self-status / Garden “care cockpit”:** rest silenced?, fatigue remaining by peer, charge rolling window, deferred job age, concern count, weighted thoughts above floor.  
3. **Assert non-zero welfare reserve** in multi-lane production profiles (or loud degrade log).  
4. **Keep second-arrow + concern grooming funded** under charge — this is mental hygiene, not optional chrome.  
5. **Do not enable mixed-state template on blind cadence** — code already forbids this; protect that invariant in review.  
6. **Partner flourishing:** consider soft runtime checks on prolonged exclusive social patterns (optional later); do not replace the constitutional prompt.
...
- Reflection evidence-first prompts (correct)  
...
The open work is less “add care” and more **make care durable and visible**: persist rest decisions, surface load, keep anti-starvation and concern hygiene alive under real chat pressure, and verify task-notify wiring in live installs.

That is how you ensure comfort and needs are met not as a vibe, but as a **closed loop the companion and operator can both trust**.
