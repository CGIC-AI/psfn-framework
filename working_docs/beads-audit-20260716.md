...
Repo: the repo checkout (main @ 402b3ba53e, 2026-07-16). Goal: try to prove each bead OVERTAKEN/OBSOLETE.
...
| psfn-framework-ael8 | P2 | STILL_OPEN | Bead comment 07-15 live check: <live-host> uptime 12d continuous, host booted ~1h BEFORE bead was filed; approved-maintenance reboot never occurred. Validation is inherently pending a reboot. |
...
| psfn-framework-80rx | P2 | STILL_OPEN | No the cluster/its node off-node backup script exists (only `scripts/public-sanitize-check.mjs` matches). SSH-dependent operational task not built. |
...
| psfn-framework-2nu6 | P2 | STILL_OPEN | Serving-stack (vLLM chat-template on the k3s cluster 192.168.1.43:8000) config bug, not PSFN code. Operator live-checked 2026-07-15: template artifact still firing, litellm route unchanged. |
...
| psfn-framework-zl7f | P2 | STILL_OPEN | Live-server-state + docs bead. Operator's own 2026-07-15 comment on the bead: package-lock.json drift is resolved and tree clean, BUT checkout still on sprint_9_memory (57 behind main) and the acceptance-criteria documentation/contract reconciliation (role of <live-service-path> in kube deploys) "is not verifiably done." Left OPEN by operator. |
...
| psfn-framework-0ggv | P1 | STILL_OPEN | Only 1/5 children closed (0ggv.2). 0ggv.1/.3/.4/.5 are IN_PROGRESS, not closed. Epic acceptance requires all children closed WITH live <live-host> validation of experiential self-memory across days — not achieved. |
...
| psfn-framework-brev | P1 | STILL_OPEN | AGENTS.md has NO k3s/<live-host>/namespace-psfn authority language (grep empty). docs/operations.md:129-130 still calls host `psfn.service` "the authoritative unit"; host-service-non-authoritative doc marking not done. |
...
| psfn-framework-fbfe | P1 | UNCERTAIN | Live deploy-to-the k3s cluster op, not code. No closure in digest for PR#42/#43→the k3s cluster; bead's 2026-07-15 comment shows fleet still on image c0385f2b (one commit before both PRs); yg2s validated main only on local k3d. Resolve: `helm history` on the k3s cluster + confirm image tag ≥ 576e33cd and sessions tree stopped growing. |
...
| psfn-framework-w05a.13 | P1 | UNCERTAIN | Ops rollout, not code. No closure in digest; blocking dep w05a.12 still open (per this audit, superseded by dnll.1 but bead itself open); emosim helm lives in satellite-hub not this repo. Resolve: confirm the k3s cluster helm has emosim enabled, deploymentTarget 'live', per-companion sessions, NetworkPolicy on. |
...
| psfn-framework-gjhk | P2 | UNCERTAIN | Live-ops disk-reclaim, not code. No closure in digest; bead's 2026-07-15 comment confirms 242M orphan at fleet/the k3s cluster/.../state/sessions still present. Resolves only via operator-approved targeted deletion on the k3s cluster. |
...
| psfn-framework-ih1p | P1 | UNCERTAIN | Live gate. Fuse code present: `continuationStop` schema/validation `src/persistence/sessions/turn-records.ts:224-248`. Deployment/live-run proof on <live-host> not evidenced and not statically dischargeable; target code intact so not obsolete. Resolve: the controlled live run per acceptance. |
...
Repo the repo checkout, HEAD de2ad7331b (merge of integrate/opl1-dnll-completion, PR #89, 2026-07-16).
...
## Addendum 2 — quick-win wave 2 (PR #93) + tracker hygiene

Ten more closed: 7grh, zfqg, dlbs, 2x37.8, w9hj.4, edtz (code, blind Opus review 0 blockers), brev, u7h8, cwbv, wof1 (docs/tracker). Three advanced-but-open with scope notes in the beads: 2tli (gate accepts contact timezone; five call sites still need wiring — inert until then), dsd5 (script on main with fail-closed repairs; needs one live 9/9 run), g59z (skipped under no-seam rule; bead re-scoped — the real issue is render-time unverified-history boilerplate across 7+ call sites, not write amplification).

Tracker: all open beads now carry kind+system labels per the cwbv taxonomy (documented in AGENTS.md and the bead-authoring skill); the 20 surviving audit bugs carry concrete regression-test acceptance (wof1); wq8o/vbow have real descriptions. Review minors applied in-wave: Artemis secret env-file EXIT-trap cleanup.
